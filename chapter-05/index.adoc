...
Le fichier `package.json` est une clé de voûte pour reproduire l'installation
...
– de simples fonctions, des bibliothèques ou des _frameworks_ complets.
Ils couvrent un spectre d'usages très larges : accès aux bases de données,
_frameworks_ web, outils _front-end_, utilitaires de test,
compression de données, paiement bancaire, des _frameworks_ mobiles, etc.
...
[TIP]
.[RemarquePreTitre]#Pratique# npm{nbsp}ci
====
...
Je peux ainsi aussi les appeler de manière individuelle.
...
_Travis{nbsp}CI_ ([URL]#https://travisci.com#).
...
- C'est une chose de plus à apprendre même si on n'a pas l'impression de l'utiliser.
====
...
C'est pratique pour identifier quel module remplacer par un autre, plus léger et donc
...
à un certain _framework_ – qu'on ne veut pas utiliser.
