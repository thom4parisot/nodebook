...
*eslint* lit par défaut le fichier de configuration `.eslintrc`, placé à la racine de votre projet mais il est tout à fait possible de lui fournir un emplacement alternatif.
Dans mon cas, j'ai indiqué une préférence pour les guillemets simples ainsi qu'une déclaration globale de `use strict` :

[source,json]
.eslintrc.json
----
include::{sourceDir}/../eslintrc.json[]
----
...
Certaines règles demandent ou acceptent des arguments supplémentaires, documentés dans le répertoire `lib/rules` de la bibliothèque _eslint_ mais aussi sur son site Web.
