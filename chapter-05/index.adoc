...
`bin`::
  Indique la présence d'<<install.global,exécutable>>.
`dist`::
  Informations à propos du fichier téléchargé si
  nous faisons `npm install <module>`.
`dependencies`::
  Les autres modules qui seront téléchargés lors de l'installation.
`dist-tags`::
  Les étiquettes définies par les auteurs et autrices, utiles que nous souhaitons
  <<install.version,jongler entre différentes versions>> du module.

Nous pouvons faire un zoom sur une métadonnée en la passant à
la commande `npm view`.
Par exemple `dependencies` pour ne lister que les dépendances d'un module donné :
...
Il est même possible d'obtenir de zoomer au niveau d'une métadonnée,
...
----
$ npm install cowsay
$ ./node_modules/.bin/cowsay Magique !
----
