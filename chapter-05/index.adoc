...
La commande `npm install` est une commande que nous utilisons quand
nous récupérons du code avec Git pour la première fois ou après une mise à jour
(avec `git pull`).

Lorsque vous tapez cette commande, l'exécutable `npm` vérifie toujours
que les modules npm installés dans le répertoire `node_modules` correspondent
bien aux modules listés dans le fichier `package.json`.
