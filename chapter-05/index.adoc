...
Si rien ne s'affiche, c'est que tout est bon : tous nos modules sont à jour !
...
Il arrive que nous ayions à utiliser une version modifiée d'un module npm
...
====
L'installation depuis un dépôt Git est plus lente que depuis un registre npm.
...
Les dépendances de développement du fichier `package.json` sont installées
uniquement lorsque la commande `npm install` est invoquée dans le même répertoire.

Ainsi si vous faisons `npm install cowsay`, seules ses dépendances normales
sont installées.
Ses dépendances de développement ne seront pas installées pour gagner du temps.

[TIP]
.[RemarquePreTitre]#Pratique# Lister les dépendances de développement d'une module
====
...
====
La <<install,commande `npm install`>> accepte une option `--production`.
