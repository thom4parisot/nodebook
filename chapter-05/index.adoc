...
[abstract]
--
Ce chapitre nous permettra d'y voir plus clair du côté des <<modules,modules npm>>.
Nous apprendrons comment identifier un bon module et comment suivre leur
évolution sans casser nos projets.
...
utilisés avec les fonctions `require()` et `import`.
Certains modules npm s'utilisent aussi comme des programmes exécutables
dans un terminal.
