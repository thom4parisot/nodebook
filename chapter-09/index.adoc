...
L'existence de Node et du registre _npm_ a favorisé le développement d'un écosystème orienté _front-end_ plus simple à appréhender.
Cela s'étend de la découverte au téléchargement des librairies tierces ainsi qu'à la compilation, l'optimisation et l'exécution des tests des applications web côté client.

Cet *écosystème rend l'écriture de code moderne normale* ; un code anticipant les futurs standards d'_ECMAScript_ et _HTML5_, sur les navigateurs actuels et anciens.
--
...
Si Node s'exécute au niveau du système d'exploitation — "côté serveur" — en quoi est-il lié au développement _front-end_ — "côté client" ?
...
Cela nécessitait systématiquement l'utilisation d'un autre environnement qu'ECMAScript lui-même: _Rhino_ nécessitait Java, _Spidermonkey_ nécessitait {cpp} et _Trident_ nécessitait un environnement Windows en plus de {cpp}.
...
Qui n'a pas déjà entamé un projet en posant la question à un client, en regardant les statistiques de trafic ou en se posant une question à soi-même : quelle est la liste des versions de navigateurs avec lequel notre site ou application web doit être compatible ?
...
Fort heureusement l'arrivée d'_esprima_ change la donne et permet d'écrire un code proche des standards qui résiste au temps.
Son existence facilite l'*émergence d'outils automatisant les transformations de code* pour satisfaire nos besoins spécifiques.
...
. les *fonctionnalités et éléments de syntaxe sont implémentés un par un*, à des vitesses différentes par les différents navigateurs web ;
. _deux tiers_ de navigateurs fonctionnent sur des *rythmes de mises à jour en cycle court* (de six à neuf semaines) – le _tiers_ restant est cadencé à *une seule mise à jour par an*.
...
----
<1> Nous déstructurons l'_agent utilisateur_ du navigateur – à noter que ce même code exécuté par Node lancerait une exception car `window` n'existe pas dans cet environnement.
...
Le _preset_ `preset-env` [URL]#https://npmjs.com/babel-preset-env# convertit
...
La configuration des _presets_ et d'autres aspects de Babel se font
...
Les *polyfills* sont des bouts de code qui nous permettent de *combler les fonctionnalités manquantes* – leur implémentation.

Un *polyfill harmonise la présence d'une fonctionnalité* au sein d'une variété de navigateurs web et d'environnements JavaScript.
Cela se fera au détriment de *quelques kilo-octets de code* à charger en plus dans nos applications.
L'appel à un service de polyfill externe entrainera un *léger ralentissement du chargement* de notre page.
...
Nous garantissons ainsi une *cohérence et stabilité de comportement* au sein de notre application, peu importe l'ordre d'exécution de nos scripts.
====
...
====
_polyfill.io_ possède une documentation très complète sur [URL]#https://qa.polyfill.io/v2/docs/#.
...
Si plusieurs scripts nécessitent des polyfills, mieux vaut inclure ces derniers _en une fois_ :
...
Cette pratique a l'avantage de _ne pas entrainer d'effets secondaires_ et de garantir le _même comportement_ dans tous les navigateurs web.
...
Nous allons maintenant chercher à transformer le fichier `script-import-jquery.js` pour à la fois rendre la syntaxe `import` intelligible  (c'est le rôle de _babel_) mais aussi pour faire le lien avec les modules _npm_ :
...
====
Les exemples suivants se basent sur l'utilisation de la librairie _jQuery_ ([URL]#https://api.jquery.com/#).
...
<3> Certaines décisions métier sont mélangées avec l'affichage de la donnée _temps_.
...
La faute n'est pas vraiment celle de _jQuery_ mais plutôt la nôtre — enfin, la _mienne_.
...
_React_ détermine les opérations à effectuer dans le document HTML en fonction de la lourdeur des opérations : (re)création complète de `<time>` dans le nœud parent, simple mise à jour d'un ou plusieurs attributs ou encore déplacement du composant ailleurs dans le document HTML, etc.
...
L'implémentation côté serveur est basée sur un serveur <<../chapter-07/index.adoc#express,Express.js>> dont l'usage est expliqué au chapitre 7.
...
Le résultat est retourné sous forme de _promesse_ (voir la <<../chapter-03/index.adoc#promise,section _Promise_>> du Chapitre 3).
...
_WebSocket_ est une technologie web favorisant les échanges bidirectionnels entre client et serveur.
...
En résumé, _WebSocket_ est idéal pour maintenir une connexion en temps-réel et pour relayer plusieurs messages à l'initiative du serveur et de tout client connecté – qu'il s'agisse d'un navigateur web ou d'un agent Node.
...
L'écosystème Node fournit énormément d'outils qui devraient nous faire *gagner du temps*, en nous aidant à exécuter des actions *lorsqu'un fichier est modifié*, en nous aidant à *organiser notre travail* mais aussi en *actualisant automatiquement notre application web* au fil du développement (fini les appuis répétés sur la touche kbd:[F5]) ou encore en *optimisant nos fichiers graphiques*.
...
Il fonctionne *exactement comme _browserify_* mais au lieu de compiler une seule fois, il compilera dès qu'un changement sera détecté – où que ce soit dans l'arbre de dépendances du point d'entrée (paramètre `-e`, `--entrypoint`).
...
Modifier un fichier. Changer de fenêtre. Recharger. Changer de fenêtre. Re-modifier un fichier. Changer de fenêtre. Recharger. _Là_ c'est bon. +
...
Node est un outil de choix lorsque l'on souhaite s'atteler au développement _front-end_ et ce n'est pas sans raison.
...
Les _scripts_ sont alors à invoquer manuellement, sur un _crochet git_
...
*diminuer le coût de maintenance de nos applications*.
...
Il faut *prévoir _au moins autant_ de tests que de branches* pour valider les
attentes.
...
* l'_exécuteur de tests_ nommé  _karma_ pour faire fonctionner la _suite de tests_ dans les navigateurs web.
...
de son comportement _avant même de l'inclure_ dans notre application.
...
----
<1> Création d'un _test_ destiné à vérifier la nature de la balise HTML à générer.
<2> Création du composant isolé (_shallow_) – aucun élément DOM ne sera créé ni inséré dans le document.
<3> On écrit une _assertion_ garantissant que l'on retourne un élément `<time>`.
...
Une fois encore des _motifs de chemins_ sont utilisés pour indiquer aux _plugins_
...
Par mesure de simplicité, cette commande a été abstraite en tant que <<../chapter-05/index.adoc#npm-scripts,script _npm_>> :
...
Notre _npm d'utilisateur_ et la _clé d'API_ doivent être renseignés en tant que
