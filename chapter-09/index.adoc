...
Les _presets_ suivants nous aideront à convertir chaque édition de spécification _ECMAScript_ vers _ECMASCript 5_.
Le *dernier _preset_ est le plus simple à utiliser*, car il se base sur les parts de marché des navigateurs web pour déterminer les règles de conversion :
...
* `preset-env` ([URL]#https://npmjs.com/babel-preset-env#).

Ils sont à placer dans un fichier de configuration nommé `.babelrc`.
L'exemple suivant illustre la _combinaison_ de deux _presets_ pour convertir la syntaxe _ECMAScript 2015_ et _ECMAScript 2016_ vers _ECMAScript 5_ :
