...
[TIP]
.[RemarquePreTitre]#Documentation# jQuery
====
...
indexterm:[application front-end, module Node]
indexterm:[Node.js, application front-end]
indexterm:[module, universel]
...
indexterm:[variable globale, fetch()]
indexterm:[variable globale, XMLHttpRequest]
...
navigateurs.
indexterm:[module npm, node-fetch]
indexterm:[module npm, whatwg-fetch]
indexterm:[ECMAScript, polyfill]

[[io-eventsource]]
=== Approche unidirectionnelle avec EventSource
indexterm:[variable globale, fetch()]
indexterm:[EventSource]
...
[URL]#https://caniuse.com#feat=eventsource#.
...
[URL]#https://caniuse.com#feat=websocket#.
...
Il est plus facile de démarrer en utilisant un module{nbsp}``npm`` _WebSocket_
comme _faye_ ([URL]#https://npmjs.com/faye-websocket#) ou _socket.io_
...
indexterm:[HTTP, code de réponse, 1xx]
indexterm:[HTTP, en-tête, Upgrade]
...
indexterm:[application front-end, rechargement continu (live reload)]
indexterm:[application front-end, remplacement de module à chaud (HMR)]
...
associées change.
indexterm:[module npm, browser-sync]
...
[TIP]
.[RemarquePreTitre]#Documentation# browser-sync
====
...
indexterm:[module npm, livereactload]
indexterm:[module npm, browserify]
...
indexterm:[application front-end, préfixes CSS]
...
indexterm:[Sass]
indexterm:[application front-end, feuille de style]
...
indexterm:[module npm, imagemin-cli]
indexterm:[module npm, imagemin]
...
indexterm:[module npm, sharp-cli]
...
[[testing]]
== Tester son code
indexterm:[application front-end, test (voir test)]
indexterm:[test]
...
La section `"<<../chapter-05/index.adoc#test,Exécuter des tests"`>> du chapitre{nbsp}5
décrit la mécanique de base pour tester son projet.
Dans cette dernière section, nous chercherons à comprendre comment appliquer
cette même logique à du code destiné aux navigateurs web.
Ces connaissances vont nous aider à trouver des bugs, à améliorer l'organisation
de notre code et à en faciliter sa maintenance.
...
Suite de tests::
  indexterm:[test, suite de test]
  indexterm:[test, unitaire]
  indexterm:[test, fonctionnel]
...
- _chai_ : [URL]#https://chaijs.com/api/bdd/# ;
...
indexterm:[test, composant React]
indexterm:[test, front-end (sans navigateur)]
indexterm:[module npm, React]
indexterm:[test, unitaire]
...
indexterm:[module npm, chai-enzyme]
indexterm:[module npm, chai]
indexterm:[test, assertion]
...
les modules{nbsp}``npm`` `karma-browserify`, `karma-mocha` et `karma-fixture` :
...
indexterm:[déploiement, intégration continue]
indexterm:[application front-end, compatibilité navigateur]
indexterm:[test, front-end (avec navigateur)]
...
On évite ainsi les oublis tout en enlevant le coût de mise en place de
...
indexterm:[déploiement, intégration continue]
indexterm:[test, intégration continue]
