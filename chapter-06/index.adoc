...
Importer du code depuis un dépôt Git déploie exactement ce qu'on veut,
sans rien oublier et avec la possibilité de revenir en arrière plus facilement.
...
Il suffit de taper `now` pour envoyer les fichiers, installer les dépendances
et obtenir l'URL à laquelle visiter l'application :

----
# Lors de la première utilisation du service sur un ordinateur
$ npm install -g now
$ now login
