...
[TIP]
.[RemarquePreTitre]#Alternative# Zeit Now
====
Zeit édite un service de _déploiements immutables_ nommé _now_.
Ce mécanisme permet de créer une nouvelle instance d'application par déploiement et de désigner celui qui correspondra à l'instance de production.

En {revisionYear}, _now_ est "juste" capable d'héberger des sites statiques, des applications web Node et des conteneurs Docker.
Il faut passer par des plates-formes de services complémentaires telles que des bases de données, de la persistance de fichiers, de l'emailing transactionnel etc.
...
- [URL]#https://www.gandi.net/hosting/iaas#
...
Retenez une chose : *ne démarrez pas une application directement sur le port 80 ou 443* — ports utilisés par convention pour parler respectivement les protocoles HTTP et HTTPS.
