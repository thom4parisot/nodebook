...
* dans *quel ordre* (quand le réseau est disponible, après un ou plusieurs autres services comme un serveur HTTP et une base de données) ;
...
Ces informations sont déclarées dans un fichier de configuration, souvent à raison d'un par programme.
La syntaxe de configuration varie selon les gestionnaires de processus mais la philosophie reste identique.

_Upstart_ est l'outil de gestion de processus de la distribution link:http://www.ubuntu-fr.org/[Linux Ubuntu].
Ses fichiers de configuration de services sont des fichiers textuels situés dans le répertoire `/etc/init` et suffixés avec l'extension `.conf`.

[source]
.upstart/nodebook.conf
----
include::{sourceDir}/upstart/nodebook.conf[]
----

L'exemple précédent décrit l'initialisation notre service après le démarrage du programme en charge des interfaces réseau (`network-services`).
Il exécute la commande `npm start` dans le répertoire précisé par la directive _Upstart_ `chdir`. +
Enfin, _Upstart_ redémarrera le service en cas d'erreur et ce, avec une limite de 5 tentatives en l'espace de 2 minutes (120 secondes).

Si le fichier de configuration est accessible en tant que `/etc/init/nodebook.conf`, le service `nodebook` peut être démarré manuellement comme suit :

----
$ sudo service nodebook start
----

D'autres commandes comme `reload`, `stop` et `restart` nous donnent le contrôle sur le cycle de vie de notre service. +
Dans tous les cas, le service sera démarré automatiquement au prochain démarrage du système d'exploitation.

[TIP]
.[RemarquePreTitre]#Documentation# Upstart
====
