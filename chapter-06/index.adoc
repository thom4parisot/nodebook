...
La commande `heroku apps:create` crée une nouvelle chez Heroku.
On pourrait faire la même chose avec un navigateur web en nous rendant sur
[URL]#https://dashboard.heroku.com/new-app#.
La commande `heroku git:remote` associe notre copie à cette application :
...
Cet exemple part du principe que notre terminal est positionné dans un
...
Dans le cas d'Heroku, la commande `heroku git:remote` crée un `remote` nommé
`heroku`.
