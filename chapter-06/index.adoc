...
image::images/pm2-status.png[align="center",width="85%"]
...
C'est le cas notamment des hébergements de type _Platform as a Service_ (_PaaS_).
...
Ils se baseront souvent sur le <<../chapter-02/index.adoc#npm-scripts,script `npm start`>> et d'autres informations contenues dans le fichier `package.json`.

Nous aborderons les principes de fonctionnement des <<paas,_Platform as a Service_>> dans une section ci-après.
...
En {revisionYear}, aucun service d'hébergement mutualisé ne proposait de mécanisme pour déployer une application Node.

Les hébergements mutualisés sont bon marché car ils se "contentent" de mutualiser un frontal HTTP et de servir des fichiers statiques et des scripts interprétés (écrits en PHP par exemple).

Ce modèle est incompatible avec le démarrage d'un processus par application Node.

[[paas]]
=== Plate-forme de services (_Platform as a Service_)

Les plates-formes de services *automatisent la configuration* et *facilitent le déploiement* de nos applications Node mais également Ruby, Python et PHP.
Elles se spécialisent dans les déploiements rapides, l'allocation des ressources et le passage à l'échelle des applications.
Leur philosophie est de *tout penser en terme de ressources et de services*.

Chaque instance peut être dimensionnée en _puissance_ (de peu de CPU à beaucoup de CPU) et en _nombre_ (de 1 à plusieurs instances en parallèle).
La plate-forme gère la distribution équilibrée du trafic web.
Des *ressources complémentaires* sont disponibles séparément et à la demande : bases de données MySQL ou postgreSQL, envoi d'emails, moteur de recherche, monitoring, aggrégation des logs etc.
...
. la plate-formùe prépare la mise en ligne en installant les dépendances _npm_ et en démarrant l'application web ;
. notre application est fonctionnelle quelques secondes plus tard.

Voici une sélection de services en ligne fournissant des plates-formes à la demande :

- [URL]#https://zeit.co#
- [URL]#https://clever-cloud.com/#
- [URL]#https://gandi.net/hosting/simple#
- [URL]#https://dashboard.heroku.com/#


La startup californienne Zeit ([URL]#https://zeit.co#) édite un service de _déploiements immutables_ nommé _now_ ([URL]#https://zeit.co/now#) qui est focalisée sur l'hébergement de sites statiques, d'applications web Node et de conteneurs Docker.

Sa particularité est de créer une *nouvelle instance d'application par déploiement* — à nous de désigner le déploiement associé à l'instance de production.
