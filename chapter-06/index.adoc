...
Ma préférence va vers _Ansible_ car il s'installe facilement sur macOS et Linux,
se configure avec une syntaxe que je connais déjà (_YAML_) et j'ai trouvé
les messages d'erreurs informatifs.
...
de l'application (dépendances, logiciels).
Le mécanisme d'exécution aide à la fois à orchestrer plusieurs images entre elles
— y compris bases de données et moteurs de recherche — et de pouvoir
revenir dans le même état applicatif qu'au démarrage de celle-ci.

Voici un exemple d'image Docker :
...
Nous pouvons constater que l'on choisit un environnement Node initial (`FROM`),
puis un répertoire de travail avant de procéder à la copie des fichiers
applicatifs vers l'image (`COPY`).
...
Nous pouvons la tester sur notre ordinateur si Docker est installé :
...
Il est aussi capable de déployer une application à partir d'un fichier `Dockerfile`
en utilisant l'option `--docker` :
...
([URL]#https://hub.docker.com#), la plate-forme officielle de partage d'images
Docker.
Elle dispose notamment d'une fonctionnalité d'empaquetage automatique via
GitHub.
À chaque nouveau commit, Docker Hub construit l'image et la met à disposition.

.Création d'un _build_ automatisé à partir d'un dépôt GitHub.
image::images/docker-automated-build.png[width="85%"]

Il ne reste alors plus qu'à la collecter sur votre ordinateur, serveur hébergé
ou <<deploy.ci,service d'intégration continue>>.
