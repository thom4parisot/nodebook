...
----
<1> Le module npm [URL]#https://npmjs.com/pokemon-random-name# exporte une
fonction qui retourne un nom aléatoire de Pokémon.
<2> `exports.endpoint` est spécifique à RunKit et accepte une fonction
identique à l'événement `server.on('request')` du
...
Sa fonctionnalité _déploie_ l'application à chaque nouveau commit sur une branche
d'un projet auquel on participe.
...
Il simplifie la <<deploy.push,configuration de Git>>, nous informe de l'état
de nos applications et d'augmenter ou de réduire la capacité des ressources
informatiques. +
L'installation se déroule depuis
...
La commande `heroku apps:create` crée une application en ligne de commande.
On pourrait faire la même chose avec un navigateur web en nous rendant sur
[URL]#https://dashboard.heroku.com/new-app#. +
La seconde étape configure le projet Git du répertoire en cours.

.Configuration de notre dépôt Git pour en faire une application Heroku.
----
$ heroku apps:create mon-application --region eu
$ heroku git:remote --app mon-application
----

Ainsi, notre projet Git contient les informations nécessaires au déploiement.
Il ne nous reste plus qu'à <<deploy.git,pousser les changements>>.
...
Cette méthode de déploiement est celle privilégiée par les
<<paas,plates-formes de services>> comme Heroku, Gandi et Clever Cloud.
Chaque projet d'application sur ces plates-formes crée aussi un dépôt Git distant
(_remote_) : un dépôt est utilisé pour versionner notre code (GitHub par exemple)
tandis qu'un autre dépôt est utilisé pour réceptionner le code de
l'application à déployer.

Illustrons un déploiement avec Heroku.
L'exemple part du principe que notre copie locale du projet Git a été
configurée avec l'<<deploy.cli,outil de déploiement>> Heroku
(cf. section précédente).

Nous pouvons nous assurer que notre copie locale est configurée à l'aide de la
commande `git remote` :
...
Heroku déploie notre application dès que la plate-forme détecte que nous
lui poussons du code :
...
En cas d'erreur, la précédente version de l'application reste en ligne.
Nous avons ainsi le loisir de corriger le problème sans interrompre le service.

[[deploy.clone]]
=== En faisant `git pull` lors d'une session SSH

La récupération du code source à distance avec Git et SSH est la manière
de déployer qui correspond le plus de notre manière de gérer
le code sur notre ordinateur.
