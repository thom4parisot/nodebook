...
_pm2_ ([URL]#hhttps://pm2.keymetrics.io#) est un gestionnaire de processus
...
<2> L'application sera relancée en cas de plantage — maximum 5 fois plantage dans un délai de 120 secondes.
...
Un serveur d'application web est un logiciel informatique
...
Quand vous obtenez la votre, modifiez la ligne de commande suivante afin
...
_New Relik_ s'installe gratuitement et en quelques clics sur la
...
Selon la nature de la faille, une personne peut l'exploiter avec un bout de code
écrit exprès pour ça pour ralentir ou faire planter notre application.
Elle pourrait aussi accéder à l'ordinateur qui fait tourner l'application,
à la base de données ou à d'autres informations confidentielles.

La seule solution : *déployer à nouveau l'application*.
Pour savoir quand déclencher le ou les déploiements,
je vous encourage à recevoir des alertes.

[horizontal]
Par e-mail::
  En vous abonnant au groupe [URL]#https://groups.google.com/group/nodejs-sec#.
Lecteur RSS::
  En vous abonnant au flux [URL]#https://nodejs.org/en/feed/vulnerability.xml#.
