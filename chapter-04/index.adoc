...
Si on ouvrait le fichier `stdout.txt` nouvellement créé, on verrait
la liste de nombre générée par notre programme. +
La *redirection a écrit le contenu dans le fichier indiqué* au lieu du terminal.
...
*Les données peuvent aller et venir* depuis et vers programmes externes,
