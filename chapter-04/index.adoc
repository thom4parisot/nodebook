...
[[process.on]]
==== Écouter les événements extérieurs

Un processus système reçoit et émet des données mais il peut aussi
*recevoir des signaux*.
D'ailleurs, vous vous souvenez de la combinaison de touches kbd:[Ctrl+C]
utilisée dans les exemples précédents ?
Cette combinaison émet un signal d'_interruption_
— dont l'identifiant est `SIGINT`.

Node gère ces signaux pour nous.
Mais on peut aussi se mettre à les écouter, auquel cas Node
nous laisse faire.

On pourrait par exemple afficher l'heure de l'arrêt du processus avant de rendre
la main :
...
Autrement dit, Node met à disposition la fonction `process.on` pour écouter
des événements.
Node nous les relaie si on les écoute sinon il les gère de lui-même.
Au-delà de `SIGINT`, nous pouvons écouter les signaux suivants :

[cols="1,2",options="header"]
|===
| Signal      | Raison du déclenchement
...
`SIGKILL` est un autre événément mais on ne peut _pas_ l'écouter.
