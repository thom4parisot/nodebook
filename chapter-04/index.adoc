...
Avant l'apparition de Node, rare était l'outillage n'imposant pas une plateforme de développement entière.
Il faut se contenter de programmes tels que _YUICompressor_, _Google Closure Compiler_, _sprockets_ ou encore _pngquant_.
Leur installation requiert des dépendances variées comme Ruby, Java ou des librairies, parfois incompatibles entre elles et dont la procédure d'installation diffère selon les systèmes d'exploitation — lorsqu'ils sont multi-plateformes.
...
Node homogénéise cet outillage et ce, de la découverte et le téléchargement des bibliothèques tierces à la compilation, à la phase d'optimisation et à l'exécution des tests de vos composants et applications web.
...
==== Unidirectionnel : XMLHttpRequest

TBD.

==== Serveur à client : Server-sent Events (SSE)
...
=== Au-delà du HTML : audio, 2D et 3D dans le navigateur
...
==== Effectuer une requête HTTP de manière universelle
