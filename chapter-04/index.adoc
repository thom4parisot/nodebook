...
Node est un outil de choix lorque l'on souhaite s'atteler au développement _frontend_ et ce n'est pas sans raison.
...
- *destrutive* : poids réduit au maximum, possibles artéfacts visuels, destruction potentielle de couleurs dans le cas d'images complexes ;
...
On voudra par exemple redimensionner des photos depuis des fichiers originaux, générer des *vignettes* ou encore *dériver plusieurs tailles d'images* adaptées aux différentes dispositions d'un site web _responsive_.
...
L'utilisation d'un <<../chapter-03/index.adoc#npm-scripts,script npm>> (cf. Chapitre 3) est idéale pour décrire les différentes actions d'optimisation.
...
Certains outils offrent une écriture d'assertion plus fluide, d'autre spécifiquement adaptée.
...
Que dire d'anciennes versions de navigateurs Android dont la rapidité de développement et donc la compatibilité est bien en deça des versions de Chrome pour ordinateur ou mobile.
