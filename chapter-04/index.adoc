...
----
<1> Affiche `https://thom4.net/#top` – le chemin et le fragment ont été modifiés.
...
----
<1> Affiche `https://thom4.net/` – les identifiants, l'ancre et les arguments ont été retirés par la fonction `format()`.
...
----
<1> Affiche `https://apprendre-nodejs.fr/v1/`.
<2> Affiche `https://thom4.net/`.
...
----
<1> Affiche `https://thom4.net/?search=node.js` – représentation de l'URL complète.
<2> Affiche `search=node.js` – représentation des paramètres seulement.
<3> Affiche `https://thom4.net/` – le paramètre `search` et sa valeur ont été supprimés de{nbsp}l'URL.
...
====
Au début de l'informatique contemporaine, les systèmes étaient conçus
...
`response.on('close')`::
  Se déclenche lorsque la requête a été terminée par le client,
  avant qu'on ait pu transmettre l'intégralité des données.
`response.on('finish')`::
  Se déclenche après l'envoi du dernier morceau de données.
`response.end()`::
...
PING thom4.net (185.31.40.11): 56 data bytes
64 bytes from 185.31.40.11: icmp_seq=0 ttl=56 time=23.763 ms

--- thom4.net ping statistics ---
...
L'absence de `.catch()` provoque un plantage applicatif et le déclenchement
