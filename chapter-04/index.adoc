...
<6> Le _plugin jQuery_ est appliqué à toutes les occurences de `<time>` dans lequel le _plugin_ est exécuté.
...
Sa documentation offrent de bons exemples pour se familiariser avec son utilisation.
...
Un _composant_ est responsable de deux choses : la *représentation de données* et la *réaction à des événéments*.
...
<6> Nous destructurons la valeur connue de `tickData` pour effectuer une opération qui nous rappelle les différents appels à `.attr('class')` et `.text()` de _jQuery_.
