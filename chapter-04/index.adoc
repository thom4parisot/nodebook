...
Avoir permis d'*écrire des modules en _ECMAScript 2015 avant que la spécification ne soit entièrement terminée*.
Et donc de pouvoir anticiper son apprentissage tout en mettant le langage à l'épreuve avant sa finalisation.

Il a depuis été dépassé en terme d'intégration, de modularité et de complétion d'implémentation de fonctionnalités _ECMAScript 2015 par <<babel,Babel>> (voir ci-après).
