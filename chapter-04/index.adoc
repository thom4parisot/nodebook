...
.Propriétés notables
`URL.format()`::
  Transforme un objet `URL` en chaîne de caractère grâce à des contrôles fins.
classe `URL`::
...
<3> Affiche `search=node.js` — représentation des paramètres seulement
<2> Affiche `https://oncletom.io/`
...
variable : recherche du serveur distant, établissement de la connexion réseau,
premier échange de données, transfert de données, clôture de la transmission.

Pour rester rapide, Node fait aussi le choix de ne pas transmettre toutes les
informations d'un coup.
L'<<events,événement>> `data` renvoie en réalité plus ou moins `10Ko` de données
à la fois.
L'exemple a fonctionné parce qu'on a eu de la chance : le fichier
`package.json` pèse moitié moins de 10Ko.
...
----
<1> Affiche un message par réception de paquet
...
Pas assez pour être décodées avec une fonction comme
<<../chapter-03/index.adoc#json,`JSON.parse`>> qui nécessite…
un document JSON complet pour être décodé.

On pourrait accumuler les morceaux de réponse et les assembler à la fin.
Cela revient à occuper autant de kilo-octets en mémoire. +
La solution n'est pas loin pourtant : il faudrait jeter un œil du côté
des <<streams,flux de données>> qui pompent et brassent les données comme du
liquide.
...
`http.get()`::
  ...
  Il s'agit d'une version légèrement simplifiée de `http.request()`.
  Elle est largement suffisante pour les besoins courants.
`http.request()`::
  Crée une connexion HTTP du type de son choix (GET, POST, OPTION, PUT, etc.).

La fonction `http.request()` nous aide à gérer plus finement notre requête.
On peut aisément régler les entêtes de requête et lire le volume de données
qui nous intéresse avant d'arrêter la transmission.

En vrai et au quotidien, on préfèrera utiliser un
<<../chapter-05/index.adoc#modules,module npm>> pour gérer ces cas de figure.
Ces modules sont bien plus fluides à utiliser que cette méthode bas-niveau
qui nous oblige à tout faire nous même.
...
`request.write()`::
  ...
  Cette méthode est appelée plusieurs fois lorsque l'on effectue un
...
Ce mécanisme se combine bien avec le <<child_process,module `child_process`>>,
