...
[[concepts]]
== Quelques concepts et spécificités de Node

WARNING: section en travaux
...
[WARNING]
.[RemarquePreTitre]#Compatibilité# Des globales pas toujours globales
====
indexterm:[module, CommonJS]
indexterm:[module, ECMAScript]
Les variables `+__filename+` et `+__dirname+` n'existent que lorsque notre code
est chargé en tant que <<modules,module CommonJS>>.
====

[[asynchronous]]
=== Opérations asynchrones
indexterm:[fonction, asynchrone]
indexterm:[programmation asynchrone]
...
C'est un prolongement du mécanisme expliqué au <<../chapter-03/index.adoc#callbacks,chapitre 3>>
Parler du pattern `fn(err, result)`, remplacé petit à petit par un mécanisme de promesses

[[buffer]]
=== Données binaires (Buffer)
...
Les modules de base contiennent des *fonctions ECMAScript pour interagir avec le système d'exploitation*.
Ils nous donnent la capacité à communiquer avec le système d'exploitation,
son système de fichiers, des ressources réseau (HTTP, DNS, TCP), entre autres.
Les modules sont inclus par défaut avec chaque installation de Node.
...
.base-module.js
----
include::{sourceDir}/base-module.js[]
----
<1> Chargement du module `fs` : ses fonctions et attributs sont désormais accessibles depuis la variable `fs` (le nom de la variable n'a pas d'incidence).
<2> Affiche un tableau qui contient toutes les fonctions et attributs du module{nbsp}``fs``.

Les modules de base évoluent au fil du temps :
...
Si les modules de base représentent la pierre angulaire de nos applications,
on peut vite se retrouver à écrire toujours et encore les mêmes choses.
L'écosystème communautaire des <<../chapter-05/index.adoc#modules,modules{nbsp}``npm``>>
étend les modules Node avec des fonctionnalités manquantes,
des utilitaires qui simplifient la vie mais aussi avec des choses qui
dépassent nos connaissances et qui n'ont pas leur place dans les modules de base.
On entrera dans les détails dans le <<../chapter-05/index.adoc#,chapitre{nbsp}5>>.
...
[NOTE]
.[RemarquePreTitre]#Notation# Répertoire courant et parent
====
Lorsqu'on écrit des chemins d'accès, le point{nbsp}(`.`) fait référence au
_répertoire courant_ et les deux points (`..`) au _répertoire parent_.

La notion de _courant_ correspond à l'emplacement depuis lequel nous
...
Le module{nbsp}``fs`` est incontournable.
On y a recours dès que l'on a besoin de lire, écrire ou déplacer un fichier,
parcourir des répertoires et surveiller leurs changements.

Toutes les fonctions dont le résultat n'est pas instantané sont <<asynchronous,asynchrones>>.
...
----
<1> La fonction `fs.readdir()` demande au système d'exploitation la liste des fichiers du répertoire courant (`.`) et exécute la <<callbacks,fonction de rappel>> quand les résultats sont disponibles.
<2> La variables `files` est un tableau dont chaque élément est un nom de fichier ou de répertoire.
indexterm:[fonction, de rappel (callback)]

Le mécanisme des <<callbacks,fonction de rappel>> est un héritage du temps
où les <<../chapter-03/index.adoc#promise,promesses>> n'existaient pas.
Fort heureusement pour nous, l'objet `fs.promises` contient une copie conforme
des fonctions historiques… mais avec des promesses. +
Reprenons l'exemple précédent pour illustrer ces propos :

[source%interactive,javascript]
.fs/intro-promises.js
----
include::{sourceDir}/fs/intro-promises.js[]
----
<1> La fonction `fs.readdir()` accepte les mêmes arguments, à l'exception de la fonction de rappel.
<2> La méthode `then()` est appelée seulement si la lecture du répertoire est un succès.

La suite de cette section repose sur l'utilisation des promesses.
J'estime qu'elles facilitent la gestion des erreurs et accentuent
à la fois la lisibilité et la maintenance du code écrit.

Je vais illustrer la gestion des erreurs en faisant appel à `fs.rmdir()`.
La suppression du répertoire courant `+__dirname+`
(voir la section `"<<globals,Variables globales>>"`) va forcément échouer
car un répertoire ne peut être supprimé tant qu'il contient un élément
...
----
<1> Affiche un message qui commence par `ENOTEMPTY: directory not empty`.
indexterm:[erreur, ENOTEMPTY]

La section `"<<errors.system,Une erreur est renvoyée par le système d’exploitation>>"`
détaille des raisons courantes d'erreurs.
Les codes associés (ici, `ENOTEMPTY`) sont des indicateurs stables et pérennes
pour détecter quel type de problème est rencontré, et quelle réponse appropriée
prévoir dans notre application.

Le <<path,module{nbsp}``path``>> est un bon compagnon au module{nbsp}``fs``.
Leur combinaison aide à construire des chemins robustes, prédictibles et
qui fonctionnent avec tous les systèmes d'exploitation.
indexterm:[module, path, join()]

Le script suivant crée un répertoire dans un <<os,dossier temporaire>>
et s'y duplique sous un autre nom :
indexterm:[module, os, tmpdir()]
indexterm:[module, fs, mkdir()]
indexterm:[module, fs, copyFile()]
...
----
<1> On assemble un chemin de fichier composé à partir du <<os,répertoire temporaire>>, fourni par le système d'exploitation.
<2> Ce message s'affiche seulement si la création du répertoire et la copie aboutissent.

Les promesses facilitent l'enchainement d'actions asynchrones.
Notre code reste relativement "plat" et explicite sur ce qu'on tente d'y faire.
indexterm:[promesse]

Une autre opération classique est de *lire le contenu d'un fichier*.
Nous ferons appel à la fonction `fs.readFile()` pour lire son contenu en une seule fois
et afficher le contenu du script que nous exécutons :

[source%interactive,javascript]
.fs/readfile.js
----
include::{sourceDir}/fs/readfile.js[]
----

À notre grande surprise, ce n'est pas le contenu du fichier qui s'affiche
mais une chaîne de caractères qui ressemble à
`<Buffer 27 75 73 65 ... 44 more bytes>`.
En effet, Node retourne le contenus sous forme binaire, avec un <<buffer,Buffer>>
(voir `"<<buffer,Données binaires (Buffer)>>"`).

Nous savons que le fichier en question est un contenu texte, nous pouvons indiquer
l'encodage de conversion souhaité.
En général, la valeur `utf8` suffit :

[source%interactive,javascript]
.fs/readfile-utf8.js
----
include::{sourceDir}/fs/readfile-utf8.js[]
----

Une fonction alternative à `fs.readFile()` existe : `fs.createReadStream()`.
Elle est adaptée aux fichiers dont le poids dépasse de la centaine d'octets.
Vous pouvez en savoir plus à son sujet dans la section
`"<<stream,stream : manipuler des flux de données>>"`.
...
`fs.mkdir()`::
  ...
  L'option `recursive` crée les répertoires parents qui manquent.
`fs.readdir()`::
...
| `ENOTEMPTY` | Le répertoire n'est pas vide. | L'opération n'aboutira qu'une fois tous ses éléments sont supprimés ou déplacés ailleurs.
...
indexterm:[Node.js, débogage, inspecteur]
