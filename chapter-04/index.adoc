...
<3> L'objet `error` contient plusieurs clés et valeurs précisant le type d'erreur,
la référence vers la ressource concernée et le nom de la fonction système
utilisée par Node pour accéder à la ressource (ici, `open`)
...
Son _code_ (`ENOENT`) signifie la même chose mais a l'avantage d'être plus
facile à vérifier dans une condition.

Ce code d'erreur est dépendant du <<modules-builtin,module Node>> utilisé.
...
notifier l'équipe de maintenance _et_
...
Le contenu de l'erreur est accessible dans le premier argument du _callback_
d'erreur.
La trace indique dans le fichier d'origine de l'erreur,
ainsi que la ligne en question.
...
<4> Dans ce cas, le dernier `.catch()` n'affiche rien car
nous n'avons pas rencontré d'autre erreur entre temps
...
Toute interface basée sur le <<events,module `events`>> peut émettre
plusieurs erreurs de manière asynchrone.
C'est le cas du <<http,module `http`>>, du <<stream,module `stream`>>
et du <<process,module `process`>> par exemple.
...
----
<1> On émet une erreur dans notre code
...
À l'instar des promesses et des callbacks, la trace d'erreurs signale
le fichier et la ligne à l'origine du problème.

Si une erreur est émise et qu'aucune fonction n'est à l'écoute,
l'événement `uncaughtException` est produit.
Suite à ça, le processus s'arrête.
...
| Retenter la connexion ; vérifier que la stabilité de la connexion réseau.
...
| Des droits administrateurs sont nécessaires pour effectuer cette opération.
C'est peut-être une opération dangereuse si cette situation est saugrenue.
...
Il arrive des fois qu'un programme ne se termine pas… alors qu'on pensait
qu'il allait s'arrêter immédiatement.

Il peut y avoir plusieurs raisons à ça :

- une ressource distante *met du temps à répondre* —
un _timeout_ déclenchera une erreur (généralement sous 30 secondes ; c'est long)
...
Pour en savoir plus, il faudra inspecter le système et observer la
