...
et nécessitent l'utilisation de l'<<option,option de démarrage>>
...
Résumons les des différences notables avec le mécanisme de modules Node :
...
le système de fichier et les accès réseaux, entre autres.

Ces modules sont inclus avec chaque installation de Node.
On peut donc en bénéficier sans effort supplémentaire.

Le chargement d'un module de base s'effectue à l'aide de la fonction `require()`.
C'est à dire de la même manière qu'avec un <<modules,module classique>>.
À une différence près : *on utilise le nom du module* au lieu d'un
emplacement fichier.

Ainsi, si l'on souhaite interagir avec des fichiers et des répertoires,
on chargera le <<fs,module `fs`>> comme ceci :
...
D'interpréteur JavaScript côté serveur, nous avons maintenant à disposition
tout ce qu'il nous faut pour créer des applications et des outils modernes.

[NOTE]
.[RemarquePreTitre]#Documentation# Modules de base
====
La documentation de chaque module est disponible sur le site officiel de Node :
[URL]#https://nodejs.org/docs/latest-{nodeCurrentVersion}.x/api/#

On y retrouve toutes les propriétés des modules de base, toutes leurs fonctions
et ce, pour chaque version de Node.
====

Les modules de base changent au fil du temps.
Des fois, de nouvelles versions de Node sortent pour corriger des bugs dans ces modules.
D'autres fois, les changements sont structurels :
ils *ajoutent des fonctions* ou en *déprécient*.

La documentation officielle de Node documente les changements,
les indices de confiance ainsi que des alternatives lorsqu'elles sont connues.

.Exemple avec la fonction `fs.exists`, dépréciée depuis Node v1.
image::images/api-deprecation-fs-exists.png[align="center",width="85%"]
...
Les sections suivantes illusteront les modules principaux avec
des cas pratiques.
