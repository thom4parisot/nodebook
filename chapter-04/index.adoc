...
====
Le contenu de ce chapitre se base par défaut sur les branches de  *Node {nodeCurrentVersion}* et *npm {npmCurrentVersion}*.

D'autres versions majeures sorties depuis ont peut-être altéré certains des comportements et fonctionnalités décrites ci-après.
====
...
npm utilise Node comme moteur d'exécution mais vous pouvez tout  fait empaqueter des images, fichiers CSS, Sass, TypeScript etc.
...
Ce projet a rencontré une forte popularité à une époque où les développeurs pensaient que _npm_ n'était pas nécessaire ni pertinent pour les dépendances _front-end_. +  
Je *déconseille fortement l'utilisation de _bower_* car il ne répond pas du tout au besoin de l'outillage _front-end_.  
Il est par ailleurs un frein majeur aux <<universal-applications,applications universelles>>.
...
Un exemple plus concret est abordé dans la section <<automation,Automatisation des tâches>>.
...
- ECMAScript, CommonJS et modules ES2015
- browserify + transform chain
