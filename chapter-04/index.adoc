...
On peut utiliser Node en parlant directement avec l'interpréteur, en créant
un script ou en programmant une application plus complexe.

Ce chapitre explique ce qu'il faut savoir pour interagir avec l'exécutable
`node`, le fonctionnement des _modules_ et les variables à disposition.
...
On peut passer des options à `format()` pour conserver ou ôter des
...
<4> Termine la connexion avec le client — ce dernier considère sa _requête_ comme terminée.
...
Si le serveur distant ne précise pas l'_encodage des caractères_,
...
----
<1> Affiche un message du genre `L'ordinateur a 4 CPU et une session ouverte par anonymous.`.
...
pour y _écrire_ du contenu et pour <<stream,rediriger leur flux>>.
...
Les flux de _sortie_ et d'_erreur_ sont manipulables en continu.
Comment ?
...
_Filtrons_ la sortie de l'exemple précédent sans écrire une ligne de code de plus.
...
<3> La _chaîne de caractères_ est transformable en majuscules, que l'on écrit dans le flux de sortie.
...
<3> Affiche `Compteur = 5` puis `Compteur = 10` (et ainsi de suite) dans la _sortie erreur_.
...
Le _code de sortie_ équivaut alors à `0`.
...
<i>ReferenceError</i>: jenexistepas is not defined
    at Object.<anonymous> (/.../examples/process/<i>exit-error.js</i>:<i>5</i>:13)
----
...
Pendant le mécanisme de modules de Node montait en puissance,
...
----
<1> On importe la _valeur par défaut_ ainsi qu'une _valeur nommée_ — c'est particulièrement pratique pour sélectionner avec finesse ce que l'on veut utiliser d'un module.
...
entre les scripts destinés au développement web frontend et les scripts Node.
...
*provoquée lorsque le code est exécuté* :
...
La ligne en-dessous documente le _type d'erreur_ (`ReferenceError`)
...
<2> L'objet d'erreur est transmis à la prochaine occurence de `.catch()`.
...
<3> Ce résultat est transmis à la prochaine occurence de `.then()`.
...
À vous de trouver _pourquoi_ un tel problème se produit.
...
| `EADDRINUSE`
| Adresse déjà utilisée : nous tentons de créer une ressource réseau alors qu'une
interface existe déjà à la même adresse.
| Vérifier l'origine du serveur déjà en place à cette adresse.
Attribuer une autre adresse/port à la ressource réseau.

| `ECONNREFUSED`
| La ressource distante a refusé la connexion.
| Vérifier si c'est normal que la ressource distante soit inactive.
Vérifier qu'on se connecte à la bonne ressource.
...
| Les systèmes d'exploitation peuvent travailler sur une quantité finie de fichiers.
Peut-être que vous avez ouvert trop de fichiers en même temps.
Fermer l'accès aux fichiers ouverts.

| `ENOENT`
| Ressource inexistante.
| Vérifier que le chemin d'accès est correct.
Inspecter la raison de l'inexistance de la ressource.
...
| Retenter l'opération. Vérifier la disponibilité de la ressource distante.
S'assurer que le volume demandé n'est pas trop important.
|===
...
majeure de Node.
Ça nous laissera le temps de corriger les erreurs avant la prochaine
mise à jour majeure.
