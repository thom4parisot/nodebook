...
Il y a plusieurs éléments à prendre en compte concernant la standardisation de nouvelles versions et fonctionnalités dans _ECMAScript_ :

. la *cadence de standardisation* a été revue pour devenir prédictible – une volonté d'une fois par an ;
. les *fonctionnalités sont implémentées une par une*, à des vitesses différentes par les différents concepteurs de navigateurs web ;
...
- *fonctionnalité non-implémentée* : transformer le code pour l'adapter aux navigateurs cibles ;
- *fonctionnalité partiellement implémentée* : utiliser l'implémentation native des navigateurs et à défaut, transformer le code pour l'adapter aux autres navigateurs ;
- *fonctionnalité totalement implémentée* : utiliser l'implémentation native des navigateurs.

Il arrive que certaines fonctionnalités soient *abandonnées* juste avant d'être standardisées – ou que leur implémentation change beaucoup (on pensera à _CSS Flexbox_).
...
L'intérêt de sa _sélectivité_ fait que l'on peut progressivement arrêter de convertir certaines fonctionnalités couvertes par 100% des navigateurs web modernes.
...
L'exemple suivant illustre un code utilisant quelques fonctionnalités _ECMAScript 2015_.
...
Voici quelques _presets_ qui vous seront utiles :
