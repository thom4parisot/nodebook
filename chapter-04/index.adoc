...
_babel_ et son _preset_ `babel-preset-env` nous facilitent l'inclusion de _polyfills_ selon la compatibilité navigateurs que l'on souhaite maintenir au sein de notre application. Le fichier suivant illustre la configuration du _preset_ à l'aide des clés `useBuiltIns` et `targets` :
...
Si le code est *redistributé* (en tant que module _npm_ public), laissons la responsabilité de _polyfiller_ aux personnes utilisant notre code.
...
==== Syntaxe de modules _ECMAScript 2015_
