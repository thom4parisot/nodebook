...
Les fonctions d'affichage du module `console` sont tellement souvent employées
que le module est préchargé par Node.
On n'a donc pas besoin de le charger manuellement avec `require('console')`.
...
`console.dir()`::
  Affichage paramétrable dédié aux objets et tableaux.
  On peut entre autres choisir la profondeur d'affichage
...
mais aussi pour l'inspecter et interagir avec du code.
