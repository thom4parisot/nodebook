...
Nous pouvons aussi exploiter ce mécanisme pour
*organiser son code dans plusieurs fichiers* et les charger depuis d'autres fichiers.
On retrouve des mécanismes comparables en Python (`import`), PHP (`require`)
...
Complétons maintenant l'usage de `require()` avec `module.exports` :
...
Cette expression permet de *rendre explicite ce qui est exporté par un module*.
...
Le code exécuté n'affiche rien en tant que tel, à part quelques messages
dans la <<console,console>>.

Un moyen d'en savoir plus sur ce qui se passe dans les entrailles d'un script
est d'utiliser l'<<inspect,inspecteur Node>>.

On peut toutefois construire des applications graphiques qui fonctionnent
dans un _terminal_.
On verra comment faire dans le <<../chapter-08/index.adoc#,chapitre 8>>. 
...
Tout type d'expression ECMAScript est acceptée en paramètre :
