...
`message.read()`::
  ...
  On apprendra mieux comment manipuler cette fonction dans la section sur le
...
Attribut `message.headers`::
  Objet contenant les en-têtes de la réponse – c'est le serveur distant qui décide
  de leur contenu.
...
De _typer_ les ressources renvoyées afin qu'elles soient comprises
par un navigateur, c'est-à-dire de pouvoir signaler que nos réponses
...
La confirmation est visuelle en visitant notre serveur avec un navigateur{nbsp}web :
...
`response.on('finish')`::
  Se délenche lorsqu'on a envoyé le dernier morceau de données au client
...
à un script Node.
On les place à la droite du nom du fichier :
