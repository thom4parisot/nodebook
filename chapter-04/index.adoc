...
.Exemple avec l'api `punycode`, dont le statut est marqué comme déprécié depuis la v7.
...
C'est aussi un environnement capable de discuter avec le système d'exploitation.
...
le système d'exploitation et les concepts d'interopérabilité
...
Il nous est également possible d'en créer de nouvelles avec une syntaxe de `CLÉ=VALEUR`.
L'exemple suivant illustre la création d'une telle variable, son affichage puis son affichage en faisant appel à un autre programme, en l'occurrence Node :
...
C'est grâce à ce mécanisme que l'exécution d'une fonction peut être reportée.
...
6. Après avoir tourné pendant 1 000 millisecondes, la boucle dépile une instruction programmée pour cet instant.
...
====
_Loupe_ est un outil de visualisation de boucle évènementielle.
...
La plate-forme Node fournit un exécutable système : `node`. C'est lui qui digèrera votre code, qu'il ait été préparé ou non :

- `node` invoquera le <<#repl,REPL>>, un interpréteur interactif (voir ci-après) ;
...
L'option `-p` est idéale pour évaluer du code en une ligne et visualiser un résultat :
...
.print-args.js
----
include::{sourceDir}/print-args.js[]
----
<1> Demande au programme `/usr/bin/env` de retourner le chemin vers l'exécutable système `node` ;
<2> Plus d'informations sur l'utilisation des `...` dans la section <<../chapter-03/index.adoc#primitive-destructuring,décomposition>> du chapitre 3.
...
Des librairies sont disponibles pour *transformer les arguments en objets JavaScript*, configurer des valeurs par défaut, de valider les valeurs, de les rendre obligatoires et même de générer automatiquement une aide à l'utilisateur.
...
----
<1> En cas de réception d'une requête HTTP sur le port `4001`, nous allons parser l'URL appelée — sauf qu'une _typo_ s'est glissée sur cette ligne (`requete` au lieu de `request`) ;
<2> Cette exception remontera jusqu'au niveau du `process` qui déclenchera l'événement `uncaughtException` ;
<3> Une seconde après avoir lancé le script, une requête HTTP est envoyée au serveur, entrainant l'exécution du _point 1_.

Encore une fois, *il vaut mieux qu'un programme plante* plutôt que de le protéger faiblement avec `uncaughtException`.

Il se peut qu'aucun code ne capture une erreur dans une <<../chapter-03/index.adoc#promise,promesse>>.
