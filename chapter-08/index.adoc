...
----
<1> L'action est le premier argument ; nous mettons les autres arguments dans un même tableau, `actionArgs`.
<2> Fonction qui retourne un nombre aléatoire.
<3> Fonction qui retourne un nombre défini de mots — 5 par défaut.
<4> Nous passons la suite des arguments à la fonction pour personnaliser l'affichage en fonction de la saisie utilisateur.

L'<<../chapter-05/index.adoc#,exécutable `npm`>> repose sur des sous-commande
...
Les arguments et les options suffisent dans des cas simples et maîtrisés.
Dans des applications plus complètes et plus complexes, la *validation*
des paramètres requiert plus de travail.
Je pense à une gestion plus fine des sous-commandes ainsi qu'à la
