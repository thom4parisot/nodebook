...
Dans cette section, nous allons couvrir la partie spécifique aux tests
d'applications en ligne de commande.
La logique reste la même : expliciter nos attentes et découvrir quand une
modification dans notre code produit un résultat inattendu.

J'utilise le module{nbsp}``npm`` _tape_ ([URL]#https://npmjs.com/tape#)
quand j'ai des besoins simples, pour démarrer.
indexterm:[module npm, tape]
J'aime son nombre réduit de dépendances — et donc sa rapidité d'installation —
ainsi que son petit nombre de fonctions d'assertion.
...
Il "`suffit`" donc d'interconnecter notre programme exécutable Node avec
le système d'exploitation — ce dernier envoie des signaux et attend des résultats
d'autocomplétion en retour.
...
Elle est un peu plus basique et demande davantage de travail pour
