...
----
<1> Crée un objet qui représente la <<../chapter-03/index.adoc#date,date>> courante – cf. <<../chapter-03/index.adoc#,chapitre 3>> pour se rafraichir la mémoire au sujet de cette API.
...
C'est l'équivalent de `+//+` en ECMAScript.
Il existe un cas spécial : lorsque le caractère `+#+` est suivi d'un `!`
...
et le reste du script est passé au programme référencé en argument – ici, `node`.
...
exécutable et prêt à le <<../chapter-05/index.adoc#publish,publier sur un registre npm>>
(cf. <<../chapter-05/index.adoc#,chapitre 5>>).
...
`--country FR --country UK`::
  Un ou plusieurs nombres ou chaînes de caractères.
`--fast`::
`--no-fast`::
`-f`::
...
C'est gênant mais ça se résout à l'aide de la propriété de configuration `validate`.
...
`<n>C`; Déplace le curseur de `<n>` cases vers la droite.
`<n>D`; Déplace le curseur de `<n>` cases vers la gauche.
|===
...
En l'occurrence ici, ça concerne nos exécutables en ligne de commande.
...
Un autre lieu courant pour documenter un projet est le fichier `README`.
