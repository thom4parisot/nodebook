...
Créer un outil en ligne de commande est un savoir utile pour se forger nos
...
systèmes d'exploitation ; une chose dans laquelle Node excelle.
...
Nous renforcerons ce programme en améliorant son expérience utilisateur
...
d'un module npm (`npm install --global <module>`).
...
Ces outils nous servent à outiller nos projets, à créer des programmes autonomes,
des interfaces visuelles dans un terminal et à automatiser ce qu'il nous semble
juste d'automatiser.
Leur distribution sur le registre npm (cf. <<../chapter-05/index.adoc#,chapitre 5>>)
les rend facile d'accès et facile à partager.
Surtout après avoir appris à <<bien les tests,tester et à bien les documenter>>.
...
Les modules npm se calquent sur son modèle de fonctionnement.
====
...
.Pendant et après la progression d'un script Node.
image::images/cli-dots.png[]

Il n'y a pas beaucoup de code à changer dans l'exemple précédent pour y
parvenir :
...
Deux modules npm simplifient la vie si les commandes ANSI ne sont pas
notre tasse de thé.
...
Le module _progress-string_ ([URL]#https://npmjs.com/progress-string#) met
...
Nous nous aidons du module npm _ora_ ([URL]#https://npmjs.com/ora#).
Il est rapide à configurer et propose une palette intéressante d'animations.

.Pendant et après la progression avec le module _ora_.
...
l'animation et en personnalisant le symbôle de réussite.
...
Cet exemple est généré à l'aide du module npm _table_ ([URL]#https://npmjs.com/table#).
...
----
<1> L'entête du tableau se distingue grâce à un style différent — du texte en gras.
<2> Chaque ligne du tableau est elle-même un tableau, à deux colonnes dans ce cas de figure.
<3> L'affichage se fait sur un simple appel de fonction — _table_ calcule la largeur des colonnes pour nous.
...
À noter que le module _table_ expose aussi une interface en flux, pour ajouter
...
L'inconvénient de l'<<../chapter-05/index.adoc#install.global,installation d'un module exécutable>>,
...
Dans cet exemple, le module _update-check_ requête le registre npm
pour déterminer la version la plus récente du module npm _nodebook_.
Si le module _update-check_ estime que la version installée localement est plus
...
<3> Un objet est retourné en cas de version plus récente ou sinon, la valeur `null`.
...
Maintenant que nous savons ça, nous pouvons présenter une information
plus digeste et actionnable à la personne qui utilise notre module :

----
$ node update/cli.js
nodebook@0.10.0 est dispo
Tape 'npm install -g nodebook'
----
...
----
<1> Nous vérifions et affichons la mise à jour si nécessaire ; nous gérons aussi une erreur (réseau indisponible, le registre npm est HS, etc.).
<2> À nous ensuite de gérer le code de notre exécutable — _parsing_ des arguments, gestion des actions, etc..

Les utilisatrices et utilisateurs sont notifiées de la disponibilité d'une mise
à jour dès qu'elles utilisent le module exécutable en question.
La décision d'effectuer la mise à jour leur appartient toutefois.
...
Un programme qui accepte de plus en plus d'options devient de plus en plus
compliqué à maintenir car il est de plus en plus dur de tester tous les cas
de figure à la main.

Un programme exécutable robuste est un script qui contient le moins de code
...
Nous avons opéré un changement : l'utilisation du `console.log` pour l'affichage
des résultats revient du côté de l'exécutable.

Notre code est portable s'il retourne un résultat et s'il laisse la
...
L'écriture de tests nous aide découvrir qu'une modification produit un résultat
différent de celui attendu.
Les tests documentent aussi les cas à la marge de notre code.
En général, dès qu'il y a un `if/else`, ça implique d'écrire au moins une
nouvelle assertion.
...
Un test s'applique à une fonction et utilise une à plusieurs assertions pour
couvrir le spectre de ses fonctionnalités.
====

Pour des besoins simples, j'utilise le module npm _tape_
([URL]#https://npmjs.com/tape#).
Il gère l'exécution des tests et il fournit quelques méthodes qui permettent
d'exprimer nos attentes — les assertions.
...
Dans cas précis, j'ai écrit deux assertions qui illustrent les différents
...
Nous sommes parés aux éventualités mais en y réfléchissant, le module `cli.js`
ne gère pas vraiment l'éventualité qu'aucun argument ne soit transmis.
...
Je trouve ça peu élégant d'être accueilli·e avec une trace d'erreur comme celle-ci.
L'erreur mériterait d'être présentée en contexte, celui de notre utilisation
et non celle des rouages internes de Node.

[[display]]
=== Présenter les messages en contexte

La présentation des messages de réussite et d'erreurs nécessite d'être à l'écoute
...
<<../chapter-03/index.adoc#promise,promesse>> (cf. <<../chapter-03/index.adoc#,chapitre 3>>).
...
d'erreur.
Nous pourrions mieux gérer certains cas de figure, comme par exemple quand
un fuseau horaire inexistant est spécifié.
...
bien de nous assurer que les conditions d'utilisation du programme soient remplies.
...
Nous avons à nouveau recourt au moduel _tape_.
...
En l'occurence ici, ça concerne nos exécutables en ligne de commande.
...
<3> Dans le cas de réussite, nous avons bien une heure qui s'affiche dans la sortie standard.

L'utilisation conjointe des tests unitaires et des tests de l'exécutable
...
et de prendre connaissance des concepts qui s'y rapportent.

Le premier endroit où documenter son application est dans l'exécutable lui-même,
avec l'option `--help` — ou son raccourci, `-h`.
...
L'aide ci-contre est minimale et n'indique que trop peu l'intention du programme.
...
Une aide qui me rassure et m'informe ressemble plutôt à ça :
...
Au premier coup d'œil, je ne décelle pas d'impasse et j'y vois plutôt une
...
Ses méthodes `.usage()`, `.example()` et la propriété `description` de chaque
...
----
<1> Documente un exemple d'utilisation et une description qui précise l'intention.
<2> Le type de l'option est rendu explicite lors de l'affichage de la documentation.
<3> La description précise l'intention de l'option pour mieux comprendre son impact si elle est activée.

[NOTE]
.[RemarquePreTitre]#Alternative# Module npm _help-version_
====
...
Un autre lieu courant pour documenter un projet est le le fichier `README`.
...
Ça tombe bien, je trouve que le `README` est un excellent endroit pour y reporter
l'affichage de la commande `--help`.

.Présentation de la documentation d'un exécutable dans un `README`.
image::images/documentation-readme.png[]

Je ne pense pas qu'il y ait besoin de faire plus compliqué pour véhiculer
...
forme que va prendre l'<<tests,écriture des tests>>.
...
web ou d'une application front-end.
...
Des modules npm comme _yargs_ ([URL]#https://npmjs.com/yargs#) aident à
...
Ce genre de modules est intéressant à utiliser quand l'assemblage d'autres
modules indépendant rend le <<interface,programme trop fragile>> et
...
Le mécanisme de concercition définit la règle de transformation d'une option
...
Options:
  --help     Affiche de l'aide               [booléen]
  --version  Affiche le numéro de version    [booléen]
  --date               [chaine de caractère] [défaut: now]
----
...
<2> Ce réglage personnalise l'affichage de la valeur par défaut dans la zone d'aide — sans ça, la date complète serait affichée, ce qui est peu élégant et moins informatif.
<3> Cette fonction s'assure que toute valeur passée en option est transformée en objet `Date`.

Autre exemple du mécanisme de coercition, cette fois-ci appliqué
...
Je vous invite à lire la section <<streaming,utiliser des flux de données>>
pour en savoir plus.
====

Cette pratique renforce une approche applicative modulaire.
L'*interface absorbe* les spécificités de la ligne de commande.
L'*application réagit aux paramètres* sans avoir à se préoccuper du contexte,
que ça soit sur le web ou dans un terminal.
...
[format="csv", options="header", cols="1,1,3"]
|===
...
`cmd <arg1|arg2>`, Alias, L'argument est obligatoire et accepte deux informations différentes — un identifiant _ou_ un e-email par exemple.
`cmd [...args]`, Tableau, Les arguments sont regroupés dans le tableau `args`.
|===
====
...
L'exemple ci-dessus illustre l'utilisation d'un chemin relatif et d'un
...
et le _globbing_.
Le module npm _glob_ ([URL]#https://npmjs.com/glob#) fait très bien ce travail
mais nous allons nous baser sur _globby_ ([URL]#https://npmjs.com/globby#) à la place.
Il supporte les <<../chapter-03/index.adoc#promise,promesses>> et je le trouve
...
----
<1> Le module _globby_ accepte un ou plusieurs motifs — ici nous n'en utilisons qu'un.
<2> Nous transformons les fichiers identifiés en chemins absolus.
<3> Pour ensuite les afficher dans le terminal.

L'expansion est caractérisée par l'utilisation des accolades (`{` et `}`).
Les valeurs sont séparées par une virgule, qui signifie _ou_.
...
Enfin, l'utilisation de la double-étoile (`**`) signifie _dans tous les répertoires_.
Nous récupérons ainsi tous les fichiers `.js` contenu dans ce répertoire
...
----
<1> Le module npm _get-stdin_ ([URL]#https://npmjs.com/get-stdin#) est pratique pour un flux d'entrée à petit volume.
<2> Le module _get-stdin_ retourne une promesse quand il a terminé de lire le flux d'entrée.
...
(cf. <<../chapter-03/index.adoc#promise,chapitre 3>>) pour transformer deux
...
La méthode `fs.createReadStream` (cf. <<../chapter-04/index.adoc#fs,chapitre 4>>)
...
----
<1> Le module npm _get-stream_ ([URL]#https://npmjs.com/get-stream#) est similaire à _get-stdin_ et fonctionne avec tout _stream_ de lecture.
<2> _get-stream_ résout une promesse dès qu'il a terminé de consommer le flux de lecture.
...
----
<1> Le module npm _through2_ ([URL]#https://npmjs.com/through2#) s'intègre à un _stream_ en exécutant une fonction à chaque afflux de données.
...
====
Certains programmes Linux utilisent le caractère `-` pour symboliser l'entrée
...
en mémoire.
En travaillant directement sur les _streams_ avec le module _through2_,
les transformations sont faites en temps réel, au fur et à mesure où les données
sont lues, bloc par bloc.

Nous sommes désormais en mesure de travailler sur avec des fichiers au même
titre qu'avec l'entrée et la sortie standard.
...
----
<1> La destination des données est choisie en utilisant un mécanisme similaire à la source de données.
...
Cette technique facilite la découverte d'une application par tatonnement.
...
Les commandes référencées ci-après ne fonctionnent peut-être pas avec.
====
...
Le mécanisme de complétion est fourni par le module npm _omelette_
...
C'est clairement pratique quand on ne sait pas par où commencer et quand on ne
connaît pas la liste exhaustive des propositions.

Nous avons parlé du module npm _yargs_ dans la section
<<application,utilisation d'un framework>>.
...
et `~/.config/fish/config.fish` pour _fish_.
====

[[packaging]]
=== Rendre le programme indépendant de Node

Nos programmes exécutables Node souffrent d'un défaut : ils imposent
...
Nous pourrions faire de même pour des ordinateurs légers de type Raspberry Pi.
...
<<../chapter-06/index.adoc#deploy.docker,hébergeurs _cloud_>>
(cf. <<../chapter-06/index.adoc#,chapitre 6>>).

[NOTE]
.[RemarquePreTitre]#Alternative# Windows sous Docker
====
Microsoft met à disposition des images de Windows Server pour Docker à cette
adresse : [URL]#https://hub.docker.com/r/microsoft/windowsservercore/#.
...
*exercice proche de celui d'une application web* écrite avec Node.
