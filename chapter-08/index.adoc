...
.Représentation des arguments et des options dans un outil en ligne de commande, une fonction et une{nbsp}URL.
|===
...
`-vv`::
  ...
  Auquel cas, la notation `--verbose --verbose` correspond à la valeur `{verbose: 2}`.
...
En l'occurrence ici, cela concerne nos exécutables en ligne de commande.
...
Les commandes référencées ci-après ne fonctionnent peut-être pas{nbsp}avec.
====
...
l'<<application,utilisation d'un _framework_>>.
