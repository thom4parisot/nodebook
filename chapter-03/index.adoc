...
- toute complexité est masquée et résumée à des commandes pouvant devenir des _conventions_ au sein de tous vos projets ;
...
====
Il se peut qu'une erreur _ EPEERINVALID_ se déclare lors d'une mise à jour ou d'une fraîche installation.
...
Pour résumer, si le module est nommé `nodebook` et est en version `1.3.0`, `npm publish` 
...
[TIP]
.[RemarquePreTitre]#Outil# david-dm.org
====
L'outil en ligne _David DM_ se charge de vérifier l'état des dépendances de chaque module publié dans le registre npm.
...
- https://nodesecurity.io/advisories (https://nodesecurity.io/rss.xml)
- http://blog.nodejs.org/vulnerability/ (http://blog.nodejs.org/feed/vulnerability/)
- http://blog.nodejs.org/ (http://blog.nodejs.org/feed/)
