...
ce que n'est _pas_ JavaScript, regardons ensemble
...
Un objet a un principe proche d'un _dictionnaire_ : on associe une valeur
...
des tweets voire des textes longs.
...
<4> Affiche un nombre aléatoire entre 0 et 100
...
`Object.assign` est une méthode qui permet d'étendre et combiner plusieurs tableaux.
...
responsable de *renvoyer du texte seulement* _et_ de *choisir les clés*
à sérialiser.
...
Ce type particulier de méthode permet de définir des _attributs_
dont la valeur est _dynamique_ (accesseur).
Le _mutateur_ gère l'affectation de valeur vers cet _attribut dynamique_.
Ces méthodes sont préfixées par `get` ou `set`.
...
<3> Affiche `"2212139938"`
— `isbn` s'utilise comme un attribut mais sa valeur est calculée à chaque fois
qu'elle est appelée
...
Le *résultat est obtenu plus tard*.
Cette résolution peut être soit _positive_ soit _négative_.
On dit alors que l'action est _asynchrone_.
...
la <<anonymous-function,fonction anonyme>>
...
----
<1> La fonction `oddTime` accepte un argument de type <<date,Date>
elle résout la promesse _positivement_ (`resolve`) si le nombre
de secondes est impair, sinon elle résout la promesse _négativement_ (`reject`)
<2> Utilisation de la forme compacte de `then` avec deux _callbacks_ :
un de succès (associé à `resolve`) et un d'échec (associé à `reject`)
...
différente
<4> Affiche `"le nombre de secondes est impair :-)"`
si la résolution est positive
<5> Affiche `"le nombre de secondes n'est pas impair :-("`
...
*en attendant l'arrivée du résultat*.
...
avec <<../chapter-04/index.adoc#util.promisify,promisify>> au chapitre 4
et dans l'<<../appendix-b/index.adoc#promise,annexe B>>.
...
----
<1> Cette fonction résout la promesse après un délai aléatoire
compris entre 0 et 2000 millisecondes
<2> On passe 3 promesses à `Promise.all`
<3> La résolution est déclenchée dès que les 3 promesses sont résolues
— l'argument contient un tableau listant les résultats
_dans l'ordre initial_ des promesses
...
----
<1> On crée une _fonction asynchrone auto-invoquée_
— c'est pas pour frimer mais parce qu'on ne peut pas encore utiliser
de fonction asynchrone directement au niveau principal d'un script
<2> Chaque utilisation de `await` met l'interpréteur en pause
<3> L'affichage du temps d'exécution de chaque promesse se fait
lorsque les 3 promesses sont résolues.

On gagne en lisibilité, mais *on perd en vitesse*.
Les promesses sont exécutées _les unes à la suite des autres_
