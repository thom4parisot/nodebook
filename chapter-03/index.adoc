...
Qu'entend-t-on par _type de données_ ?
...
Début et fin de chaîne (``^``{nbsp}et{nbsp}``$``)::
  Quand l'option multi-ligne (`m`) est utilisée, les notions de début
...
ECMAScript fournit un ensemble de fonctions de base : `console.log`, `setTimeout`, etc.
...
<3> Affiche l'année de la date passée en argument – dans cet exemple, l'année en courss.
...
À l'inverse, la méthode `join` _concatène_ tous les éléments dans une
...
Une autre syntaxe existe pour _créer_ des valeurs et y _accéder_ en utilisant
...
----
<1> Définition de l'__accesseur__{nbsp}``isbn``.
