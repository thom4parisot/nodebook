...
La portée de la variable `mot` est locale.
...
On peut y accéder, comme en atteste le de code de la fonction `nextYear`.

À l'inverse, la variable `value` qui est passée en paramètre de la fonction
`next` a une portée locale à cette fonction.
...
La portée est délimitée au bloc d'instructions dans lequel la variable
est déclarée.

Ainsi la variable `price` n'existe que dans le cadre du bloc `if`.
C'est utile pour déclarer des variables sans _polluer_ le reste du
script, pour que son existence soit oubliée aussitôt le bloc exécuté.
