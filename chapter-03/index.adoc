...
obtenues dans un navigateur web avec les fonctions `document.querySelectorAll`
et `document.getElementsByTagName`, par exemple.
...
les boucles
  une manière de parcourir plusieurs valeurs
  automatisme, éviter de répéter du code : on ne connait pas pas forcément la taille de l'ensemble
...
plusieurs méthodes
  - for..in, for..of (clé, valeur)
  - forEach (élément)
  - while (renvoi vers les conditions)

Puis des renvois vers
  - map
  - sort
  - filter
  - reduce
...
La méthode `sort` _transforme_ un tableau existant à l'aide d'une
<<anonymous-function,fonction anonyme>> qui compare les éléments entre eux.
La fonction anonyme   reçoit deux arguments, tous deux des éléments du-dit tableau.
...
[TIP]
.[RemarquePreTitre]## `Array.reverse`
====
reverse
====
...
[CAUTION]
.[RemarquePreTitre]#Attention# Gare aux valeurs vides !
====
Le comportement des différentes méthodes de tableau peut varier
selon qu'une valeur soit vide, fausse ou non-existante,
On dit alors que le tableau n'est contigu.

On peut avoir l'illusion de traiter toutes les valeurs d'un tableau
alors qu'en réalité il ne s'agit que des valeurs non-vides.

[source%interactive,javascript]
.array/non-contiguous.js
----
include::{sourceDir}::array/non-contiguous.js[]
----
...
.objects/base.js
----
include::{sourceDir}/objects/base.js[]
----


`Object.keys` liste les propriétés d'un objet
et les retourne sous forme de <<array,tableau>> :

[source%interactive,javascript]
.objects/keys.js
----
include::{sourceDir}/objects/keys.js[]
----
