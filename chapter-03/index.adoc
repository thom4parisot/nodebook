...
image::images/package-json.png[align="center",scaledwidth="85%"]
...
image::images/npm-init.png[align="center",scaledwidth="85%"]
...
image::images/npm-install-save.png[align="center",scaledwidth="85%"]
...
image::images/npm-outdated-peerinvalid.png[align="center",scaledwidth="85%"]
...
image::images/npm-outdated.png[align="center",scaledwidth="85%"]
...
image::images/npm-outdated-wanted.png[align="center",scaledwidth="85%"]
...
image::images/npm-outdated-uptodate.png[align="center",scaledwidth="85%"]
...
.npm search /slug/
image::images/npm-search-cli.png[align="center",scaledwidth="85%"]
...
image::images/module-npm-ok.png[align="center",scaledwidth="85%"]

.Page npm peu encourageante
image::images/module-npm-ko.png[align="center",scaledwidth="85%"]
...
Ces bibliothèques adressent des besoins quotidiens en terme de manipulation de chaîne ou de dates.
...
[URL]#https://npmjs.org/request#

[source,javascript]
.npm-request.js
----
include::{sourceDir}/snippets/npm-request.js[]
----
...
_eventemitter2_ est une bibliothèque performante de gestion d'événements.
...
_rimraf_ se situe dans la lignée _mkdirp_ : il supprime une arborescence de manière récursive — équivalent de la commande `rm -rf`.
...
image::images/npm-chalk.png[align="center",scaledwidth="85%"]
...
image::images/npm-cli-table.png[align="center",scaledwidth="85%"]
...
Compatible Node et navigateurs Web. +
[URL]#https://npmjs.org/faye#

[source,javascript]
.npm-faye.js
----
include::{sourceDir}/snippets/npm-faye.js[]
----
...
.stylesheet.less
----
include::{sourceDir}/snippets/stylesheet.less[]
----
...
[source]
----
include::{sourceDir}/snippets/stylesheet.css[]
----
...
Compatible Node et navigateurs Web. +
[URL]#https://npmjs.org/handlebars#
...
.Résultat de couverture de code généré par _blanket_
image::images/blanket.png[align="center",scaledwidth="85%"]
...
image::images/karma.png[align="center",scaledwidth="85%"]
...
L'utilisation de Node pour le développement _frontend_ est expliqué dans le _chapitre 5 : au service du développement frontend_.

[URL]#https://npmjs.org/grunt# et [URL]#https://npmjs.org/grunt-cli#
...
Cet exemple illustre comment compiler une extension Chrome, ainsi que ses feuilles de style LESS ([URL]#http://lesscss.org/#), à la demande et en temps réel.

Ces tâches peuvent ensuite être invoquées en intégralité ou individuellement :

----
# Exécuter toutes les tâches
grunt build

# Compiler uniquement les feuilles de style
grunt css
...
_gulp_ est un outil de _build_ — en opposition à _grunt_ qui est un outil d'exécution de tâches.
...
Le fichier de configuration de _Gulp_ décrit les séquences d'exécution nécessaires à la production de vos artéfacts :
...
Le concept de _streams_ est décrit en fin de chapitre.
Il vous aidera à mieux comprendre la magie du fonctionnement de _Gulp_.
...
- mikeal ([URL]#https://npmjs.org/~mikeal#) ;
- rwaldron ([URL]#https://npmjs.org/~rwaldron#).
...
.pattern-async.js
----
include::{sourceDir}/snippets/pattern-async.js[]
----
...
----
include::{sourceDir}/snippets/pattern-async-reusable.js[]
----
...
.get-filecontent.js
----
include::{sourceDir}/snippets/get-filecontent.js[]
----
...
.pattern-sync.js
----
include::{sourceDir}/snippets/pattern-sync.js[]
----
...
----
include::{sourceDir}/snippets/get-filecontent-sync.js[]
----
...
----
include::{sourceDir}/snippets/pattern-async-fake.js[]
----
...
----
include::{sourceDir}/snippets/get-filecontent-async-fake.js[]
----
<1> Premier accès bloquant ; 
<2> Second accès bloquant.

Le fait que Node explicite tout accès bloquant permet de repérer rapidement les potentiels goulots d'étrangement.
...
[source,javascript]
----
var Person = require('./lib/models/person.js');
var Vehicle = require('./lib/models/vehicle.js');
var Location = require('./lib/models/location.js');
var Transaction = require('./lib/models/transaction.js');

var rental = new Transaction({
  // ...
});

rental.save(function(err, commit){
  if (err) {
    throw err;
	}
...
var rental = new models.Transaction({
  // ...
});
----
...
.pattern-glob.js
----
include::{sourceDir}/snippets/pattern-glob.js[]
----
...
.app.js
----
var app = module.exports = new AppServer();
...
app.set('view engine', require('consolidate').ejs);
...
- quid de l'utilisation de ce module de configuration si l'on souhaite configurer un autre fichier qu'`app.js` ?
...
Le _pattern_ _callback_ est très couramment employé et pour cause : il est systématiquement employé dans les API asynchrones de Node.

Le _pattern_ implique une fonction ayant un ou plusieurs arguments ; le premier d'entre eux étant systématiquement un objet d'erreur. +
Ce mécanisme permet de *propager une erreur* le long de la chaîne de _callbacks_.

Forçons le trait en créant une fonction asynchrone faisant appel à deux autres fonctions asynchrones :

[source,javascript]
.message-abbr.js
----
include::{sourceDir}/snippets/message-abbr.js[]
----

Plusieurs remarques :

- `process.nextTick` est utilisé pour rendre asynchrone du code ordinairement synchrone ;
- chaque niveau de _callback_ appellera la fonction de callback initiale avec l'objet d'erreur en premier argument ;
- l'imbrication rend difficilement contrôlable la possibilité d'interagir avant que l'intégralité de la chaîne soit accomplie.

Le module suivant exécute la fonction asynchrone précédemment exportée à trois reprises, pour illustrer différents cas de figure :

[source,javascript]
.pattern-callback.js
----
include::{sourceDir}/snippets/pattern-callback.js[]
----
<1> Affiche la chaîne `GME` ;
<2> Affiche le message d'erreur `[TypeError: message is not a String]` ;
<3> Affiche une exception `TypeError: Cannot read property 'toLocaleUpperCase' of null` et termine le script de manière prématurée.
...
Et bien pour la simple et bonne raison que `try/catch` ne fonctionne que sur du _code synchrone_. +
Dans notre cas, l'exception n'étant pas interceptée dans les rouages asynchrones, elle remonte jusqu'au processus initial et génère un événement `uncaughtException`.

Autrement dit, un code asynchrone requiert encore plus de précautions du fait de la potentielle difficulté à intercepter les erreurs, et donc à éviter le plantage de notre application.
...
Le code initial peut être rendu légèrement plus lisible sans toucher à l'API de callback en utilisant la méthode `waterfall` de la bibliothèque `async` ([URL]#https://npmjs.org/async#) :

[source,javascript]
.message-abbr-waterfall.js
----
include::{sourceDir}/snippets/message-abbr-waterfall.js[]
----
...
Alors certes votre application pourra _accepter_ davantage de sollicitation mais votre code, lui, ira à la vitesse prescrite par l'enchainement logique du flux de données.
...
Les événements nous offrent un tel mécanisme d'exposition, en permettant la connection de plusieurs écouteurs (_listeners_).
...
- transmettre des résultats au travers de plusieurs couches applicatives.

Reprenons l'exemple de la section précédente à propos des callbacks.
...
Le changement le plus rapide reviendrait à transformer le code de la manière suivante :

[source,javascript]
.pattern-callback-naive.js
----
include::{sourceDir}/snippets/pattern-callback-naive.js[]
----

C'est visuellement propre mais il est difficile de savoir en quoi les fonctions exécutées dans le _callback_ sont liées.
Quid de nos besoins de maintenance et de logique applicative ? +
Sans compter qu'il est virtuellement impossible pour un autre module de greffer un quelconque traitement.

Moins de cinq minutes suffisent à implémenter un traitement événementiel dans notre module Node :

[source,javascript]
.pattern-event.js
----
include::{sourceDir}/snippets/pattern-event.js[]
----
<1> L'utilisation de `module.exports` permet d'exposer l'émetteur d'événement en dehors de notre module, par exemple `require('./pattern-events').on('data', ...)`.

Cette fois-ci notre fonction va retourner une instance de l'objet `EventEmitter`, de manière synchrone.
Le traitement reste asynchrone.

[source,javascript]
.message-abbr-evented.js
----
include::{sourceDir}/snippets/message-abbr-evented.js[]
----

On notera qu'aucun code n'est injecté dans notre module retravaillé.
Le module se contente d'exposer une API et d'émettre des données aux moments adéquats.
...
[source,javascript]
----
include::{sourceDir}/tips/events-setmaxlisteners.js[]
----
====
...
Le concept des promesses (_promises_) est extrêmement puissant pour gérer, ordonner et organiser un ou plusieurs enchaînements d'exécutions asynchrones.

Historiquement, de nombreuses bibliothèques ont proposé leur propre implémentation du _pattern_ mais sans être réellement interopérables.
La spécification _Promise/A+_ a émergé pour établir un standard de compatibilité.
Il faut attendre ECMAScript 6 pour disposer d'une API native, que ce soit pour le Web ou pour Node.
...
Faisons évoluer notre exemple de _pattern_ _callback_ et d'événement :

[source,javascript]
.pattern-promise.js
----
include::{sourceDir}/snippets/pattern-promise.js[]
----

La lecture du code est similaire à un enchaînement d'événements.
Les méthodes `then` et `catch` se chaînent mais nous verrons un peu plus tard leur subtilité.

Tournons-nous vers l'implémentation de la fonction `messageAbbr` :

[source,javascript]
.message-abbr-promisified.js
----
include::{sourceDir}/snippets/message-abbr-promisified.js[]
----

Plusieurs choses notables sont à relever :

- chaque fonction `*Async` retourne une promesse ;
- les callbacks ne sont plus des arguments des fonctions de traitement ;
- les fonctions `reject` et `resolve` remplacent le _pattern_ `callback(error, result)` ;
- l'enchaînement final de `then` laisse supposer que chaque promesse consomme le résultat de la promesse précédente;
- `messageAbbr` est elle-même une promesse dont le résultat se base sur un enchaînement de promesses.

Un atout essentiel des promesses est leur gestion des erreurs.
Et d'une manière plus élégante qu'avec les _callbacks_ : les exceptions que nous n'avons pas englobé avec des `try/catch` seront quand même capturées :

[source,javascript]
.pattern-promise-errors.js
----
include::{sourceDir}/snippets/pattern-promise-errors.js[]
----
<1> Comme dans le _pattern_ _callback_, le message d'exception est affiché `[TypeError: Cannot read property 'toLocaleUpperCase' of null]` ;
<2> Cette fois-ci, le cas d'erreur imprévu de la valeur nulle ne nous échappe pas et fait remonter une exception remontée à la fonction `logErr`.

Enfin, la méthode statique `Promise.all` nous permet d'exécuter plusieurs promesses en parallèle.
Le résultat de l'intégralité des promesses est alors retourné en argument de `then`.

[source,javascript]
.pattern-promise-multiple.js
----
include::{sourceDir}/snippets/pattern-promise-multiple.js[]
----
<1> Affiche `Promesse tenue !` ;
<2> Affiche `[ 'H&M', 'HMS', 'GO' ]`

Aussi logique que cela puisse paraître, cette technique est plus rapidement qu'un enchainement de promesse.
Le temps d'exécution correspond à celui de la plus lente promesse au lieu d'une accumulation séquentielle de temps d'exécution.
...
----
include::{sourceDir}/snippets/npm-streams-http-async.js[]
----
...
.npm-streams-http.js
----
include::{sourceDir}/snippets/npm-streams-http.js[]
----
...
----
include::{sourceDir}/snippets/pattern-streams-async.js[]
----
...
.pattern-streams.js
----
include::{sourceDir}/snippets/pattern-streams.js[]
----
...
Le fichier en question est issu de la _ressourcerie Datalocale_ ([URL]#http://www.datalocale.fr/drupal7/dataset/catalogue-datalocale#), en date du 19 mars 2014 :

[source,javascript]
.pattern-streams-large.js
----
include::{sourceDir}/snippets/pattern-streams-large.js[]
----
...
C'est exactement ce que nous propose le module npm `JSONStream` ([URL]#https://npmjs.org/JSONStream#).
...
----
include::{sourceDir}/snippets/pattern-streams-jsonstream.js[]
----
<1> Affiche `208`.
...
----
include::{sourceDir}/snippets/pattern-streams-multipipes.js[]
----
...
----
include::{sourceDir}/snippets/pattern-streams-multipipes-es.js[]
----
