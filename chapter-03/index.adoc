...
Cepndant on se rend vite compte que la simplicité du _pattern_ cache une complexité dans la gestion des erreurs.
...
L'exemple suivant repose sur les fonctions précédemment listées de `message-abbr.js`.
...
L'exception n'étant pas interceptée par notre code, elle continue de remonter jusqu'au processus initial qui émet un événement `uncaughtException`.
Processus qui se terminera avec un code d'erreur si aucune fonction n'écoute ce dit événement `uncaughtException`.
...
Mais que se passe-t'il si une de ces trois fonctions génère une erreur ? Et si l'une d'entre elle s'exécute de manière asynchrone ? Comment faire remonter l'erreur en dehors de `messageAbbr` ?
...
Nous en avons déjà parlé dans la <<../chapter-02/index.adoc#_promise,section Promise du Chapitre 2>>.

Historiquement, de nombreuses bibliothèques ont proposé leur propre implémentation du _pattern_ mais avec le défaut de ne pas être interopérables entre elles.
La spécification _Promise/A+_ a émergé pour établir un standard de compatibilité.
ECMAScript 6 introduit nativement cette API, la rendant alors disponible pour le web et pour Node, avec ou sans _polyfill_.
...
La lecture du code est similaire à celle du code écrit pour le _pattern_ d'événements.
...
- toutes les fonctions retournent désormais une promesse ;
...
Un atout essentiel des promesses est la _gestion des erreurs_.
Elle est plus élégante qu'avec les _callbacks_ : les exceptions que nous n'avons pas englobé avec des `try/catch` seront quand même capturées :
...
<2> Contrairement à notre exemple du _pattern_ _callback_, le cas d'erreur imprévu de la valeur `null` ne nous échappe pas et transmet l'erreur à la fonction `logErr` ;
<3> `logErr` ne va pas capturer l'exception lancée sur la même ligne mais uniquement de la précédente occurence de `.then()` dans la chaîne – donc uniquement si `logData` génère une exception ;
<4> Affiche `Error: Erreur volontaire dans .catch()` — il est important de conclure une chaîne de promesses avec `.catch()` pour s'assurer qu'aucune erreur ne nous échappe.

Enfin, la méthode statique `Promise.all` nous permet d'exécuter une fonction lorsque _toutes_ les promesses sont résolues.
...
Aussi logique que cela puisse paraître, cette technique est plus rapide qu'un enchainement de promesses du fait de leur _exécution en parallèle_.
Le temps d'exécution correspond à celui de la plus lente promesse au lieu d'une accumulation du temps d'exécution de chaque promesse.
