...
Il conviendrait alors de gérer le plantage d'une programme avec l'événement `uncaughtException` de la même manière que vous le feriez avec `uncaughtException`.
...
La liste détaillées des commandes supportées nativement par _npm_ est documentée sur [URL]#https://docs.npmjs.com/misc/scripts#.
