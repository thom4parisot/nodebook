...
Compatible Node et navigateurs Web. +
[URL]#https://npmjs.org/faye# et [URL]#https://npmjs.org/socket.io#
...
ECMAScript 6 introduit nativement cette API, la rendant alors disponible pour le Web ou pour Node, avec ou sans _polyfill_.
...
Enfin, la méthode statique `Promise.all` nous permet d'exécuter un _callback_ lorsque _toutes_ les promesses sont résolues.
