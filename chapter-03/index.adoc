...
La fonction `Array.from` est une autre manière de créer
...
<2> Affiche `["saturday", "sunday"]`
— le nouveau tableau est créé en sélectionnant la deuxième valeur de chaque tableau.
...
<2> Affiche `"IDNOMPRENOM"`
— sans mention de séparateur, les valeurs sont juxtaposées.
