...
_appelle le constructeur de la classe étendue_.
...
<7> Utilise la <<array-destructuring,décomposition>> pour extraire les valeurs du `Set` et affiche `[ 2, 3, [ 5, 7 ], [ 5, 7 ], Set { 11, 13 }, [ 11, 13 ] ]`.
...
- l'itération est très simple avec `for..of`, `forEach` ou même en utilisant la <<array-destructuring,décomposition>>.
...
- pas de conversion native en <<json,JSON>>.
