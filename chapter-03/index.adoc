...
La seconde installera les dépendances manquantes et enfin la troisième mettre à jour les dépendances locales.

`npm update` se comporte de manière similaire à `npm install` à une différence près : seules les dépendances préalablement installées seront mises à jour, et seulement si nécessaire.

Ces trois commandes vous assureront un état d'installation similaire à ce qu'une personne aurait en procédant à une installation _fraîche_.
