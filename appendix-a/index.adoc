...
à laquelle vous réferer pour démarrer rapidement mais aussi pour mieux comprendre
tous les contextes d'utilisation de Node.

J'ai bâti cette liste sur mon expérience personnelle et sur la confiance que je
porte aux personnes à l'origine de ces modules.
...
Elles vous permettront aussi d'organiser votre code en exécutant des
fonctions de manière séquentielle, parallèle ou en boucles asynchrones.

=== lodash

_lodash_ ([URL]#https://npmjs.com/lodash#) est une excellente librairie
de manipulation de tableaux, d'objets, de fonctions et de chaînes.

Vous pourrez ainsi boucler, filtrer, extraire et composer sans vous soucier
de la compatibilité de l'environnement d'exécution.

Ce module est compatible avec Node et les navigateurs Web.
...
_string_ ([URL]#https://npmjs.com/string#) est un utilitaire de
transformation de chaînes de caractère.
...
_eventemitter3_ ([URL]#https://npmjs.com/eventemitter3#) est une librairie
performante de gestion d'événements.
