...
de migrations pour MySQL, SQLite, MongoDB et Postgres.
À l'aide de scripts de migration, il rend reproductible des changements
...
Le module `assert` intégré à Node peut suffire à démarrer.
...
_chai_ ([URL]#https://npmjs.com/chai#) est une librairie poussée d'assertions.
...
tout en observant le comportement réel de notre code.
...
intègre _sinon_ avec le mécanisme d'assertions de <<chai>>.
...
_nyc_ se greffe sur n'importe quel librairie d'exécution de tests.
Sous le capot, c'est le module _istanbul_ ([URL]#https://npmjs.com/istanbul#)
qui est utilisé pour effectuer les calculs de couverture de code.

.Couverture de code visualisée dans un navigateur web.
...
.Exemple d'exécution de tests en continu avec Karma.
image::images/karma.png[width="85%"]

=== supertest

_supertest_ ([URL]#https://npmjs.com/supertest#) nous permet de tester
une interface HTTP.
...
nano-ordinateurs, frigo et autres ampoules connectées.
...
_serialport_ ([URL]#https://npmjs.com/serialport#) est la librairie de base
...
Le projet émane d'employé·e·s d'IBM.
...
L'écosystème npm a participé à un essor d'outillage lié à l'accessibilité
d'applications web.
...
l'ouvrant à un public trop souvent mis de côté.
...
Ce module npm le rend utilisable depuis un terminal et écrit le rapport
dans un fichier HTML.

=== storybook-addon-a11y

_storybook-addon-a11y_ ([URL]#https://npmjs.com/storybook-addon-a11y#)
est une extension au module <<storybook>>.
