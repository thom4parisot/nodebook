...
Ces arguments sont documentés dans le répertoire `lib/rules` de la librairie _eslint_ ainsi que sur le site web du projet ([URL]#http://eslint.org/docs/rules/#).
...
*uncss* ([URL]#https://npmjs.com/uncss#) est le pendant inverse d'_autoprefixer_
...
Si le monde de l'optimisation ECMAScript a explosé depuis l'apparition d'*esprima*,
il en est de même pour CSS avec *PostCSS* ([URL]#https://npmjs.com/postcss#). +
...
à leurs fins. +
Avec une ou plusieurs extensions, vous avez ainsi accès à *autoprefixer*, à
...
Ces librairies adressent des besoins quotidiens en termes de  manipulation de chaînes ou de dates.
...
_eventemitter3_ est une librairie performante de gestion d'événements.
...
_LevelDB_ est un système de base de données clé/valeur extrêmement modulaire, avec un stockage sur disque par défaut mais qui peut évoluer vers du stockage en mémoire (module `memdown`).
...
image::images/npm-chalk.png[width="85%"]

=== table

_table_ est un utilitaire qui transforme un tableau d'objets ECMAScript
en un tableau visuel ASCII.
dans le terminal.

Des options vous permettent de gérer la largeur des colonnes, les alignements
de texte et les troncatures, entre autres.

[URL]#https://npmjs.com/table#

[source,javascript]
.npm/table.js
----
include::{sourceDir}/npm/table.js[]
----
...
_nunjucks_ est un portage de _Jinja_, librairie bien connue par
...
_supertest_ est une librairie de tests pour API HTTP.
...
La librairie se base sur un mécanisme déclaratif de fonctionnalités et de terminaux ainsi que la publication de messages.

[URL]#https://npmjs.com/nitrogen#

=== serialport

_serialport_ est une librairie pour communiquer avec tout appareil bénéficiant d'un port série et relié à un périphérique via un port USB.

[URL]#https://npmjs.com/serialport#

=== firmata

_firmata_ est un _firmware_, un protocole de communication ainsi qu'une librairie de communication pour microcontrôleur Arduino.
...
_johnny-five_ est une librairie haut niveau pour Arduino, Raspberry Pi et autres microcontrôleurs.
...
_cylon_ est une autre librairie haut niveau orientée vers la programmation d'interactions pour robots.
