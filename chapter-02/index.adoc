...
De même, taper `nvm use default` ou `nvm use v8` nous fera basculer vers la branche la version v{nodeV}.
...
L'installation de node {v}.x sous Ubuntu et Mint devient aussi simple que les deux commandes suivantes :
...
Nous nous repencherons sur Docker dans la section  <<../chapter-06/index.adoc#immutable-deploy,déploiement immutable>> du Chapitre 6.
