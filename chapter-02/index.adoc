...
Une liste complète mais non exhaustive est mise jour sur le wiki du dépôt GitHub de Node à cette adresse : [URL]#https://github.com/nodejs/node/wiki/Installing-Node.js-via-package-manager#.
...
====
Les installeurs, les binaires et les sources de Node sont disponibles sur le site officiel [URL]#nodejs.org#. +
Si vous vous sentez perdu(e), téléchargez l'installeur adapté, ouvrez un terminal et sautez quelques pages pour plonger dans l'utilisation de Node.
...
Des binaires et installeurs sont de ce fait disponibles pour Raspberry Pi, à base de processeur _ARM_, et très probablement pour d'autres plates-formes de ce genre.
...
Ses forces résident dans sa relative légèreté (en comparaison à son concurrent _Eclipse_), une auto-complétion intelligente prenant en compte les modules CommonJS et AMD, une intégration des outils populaires dans l'écosystème Node (npm, JSHint, Mocha, Karma, Bower etc.) et un débogage avancé.
...
- [URL]#https://sublimecodeintel.github.io/SublimeCodeIntel/#
- [URL]#https://sublime.wbond.net/search/node.js#
...
- [URL]#https://github.com/nodejs/node/wiki/Vim-Plugins#
...
image::images/compat-table.png[align="center",scaledwidth="85%"]
...
image::images/compat-table-node.png[align="center",scaledwidth="85%"]
...
$ node --harmony-async-await script.js    # <2>
----
<1> Active toutes les fonctionnalités préliminaires ;
<2> Active uniquement la fonctionnalité <<primitives/async-await,async/await>>.
...
Les éléments retournée par les _paramètres du reste_ interface ont aussi l'avantage d'être un véritable objet de <<primitives/array,tableau>> – `arguments` est _itérable_ mais n'est pas un tableau, et donc ne bénéficie pas des méthodes de compréhension telles que `forEach`, `map`, etc.
...
Nous avons vu comment exposer une fonction à _callback_ existante en promesse mais en réalité, ce n'est pas un travail que nous avons envie de faire régulièrement.
Pour celà, une librairie comme `bluebird` ou `pify` exposent une fonction pour transformer en promesse une fonction ou un module retournant un objet de fonctions. +
La réécriture de notre exemple précédent en serait simplifié car elles nous affranchirait du fichier `./readfile-promise.js` :

[source,javascript]
.primitives/promise-all-promisified.js
----
include::{sourceDir}/primitives/promise-promisified.js[]
----
<1> Les méthodes du module `fs` sont transformées en promesses et nous en décomposons la fonction `readFile` en la renommant en `readFilePromise` ;
<2> Résultat identique à celui calculé dans `callback-hell.js` et `primitives/promise-all.js`.
...
Ils sont accessibles dans Node à partir de la v7 et nécessitent l'activation d'une option spécifique :

----
$ node --harmony-async-await un-script.js
----
...
<5> <2> Résultat identique à celui calculé dans `callback-hell.js`, `primitives/promise-all.js` et `primitives/promise-all-promisified.js`.
...
- `--harmony` (ou tout sous-ensemble de type `--harmony-async-await`, `--harmony-xx`) : active des fonctionnalités expérimentales ;
...
C'est une opération utile pour rendre un programme accessible s'il est situé dans le `$PATH` système ou pour rendre un programme accessible lors de l'installation globale d'un _paquet npm_, entre autre.
