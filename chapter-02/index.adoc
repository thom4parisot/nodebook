...
- Si vous vous êtes déjà servi 'un terminal : je recommande d'utiliser <<nvm,nvm>>.
...
Il est facile de s'essayer à Node juste avec un navigateur web moderne comme
...
logiciel _open source_ Eclipse et édité par la compagnie Red{nbsp}Hat.
...
dont EC2 et <<../chapter-06/index.adoc#hosting.lambda,Lambda>>
...
Rendez-vous dans la section <<install-macos,macOS>> pour en savoir{nbsp}plus.
====
...
dossier kbd:[Node.js] du kbd:[menu Démarrer] :
...
- kbd:[Node.js] : un terminal pour jouer avec JavaScript et voir les résultats que
Node va interpréter ;
- kbd:[Node.js command prompt] : un terminal pour exécuter nos programmes écrits
en JavaScript.
...
Docker ([URL]#https://docker.com#) est un outil _open source_ dit d'_applications en conteneur_.
...
kbd:[Debian] (`node:{v}-wheezy`)::
  Comme kbd:[standard] mais sur des bases Debian différentes, comme Wheezy, Stretch, etc..
kbd:[Alpine] (`node:{v}-alpine`)::
  Distribution spécialement créée pour Docker ([URL]#http://alpinelinux.org#), qui pèse à peine quelques{nbsp}Mo.
kbd:[Allégée] (`node:{v}-slim`)::
...
macOS::
* Terminal.app : fourni par défaut (dans kbd:[Applications / Utilitaires]) ;
...
Windows::
*_Node.js Command Prompt : fourni avec l'<<windows,installeur Windows>> ;
...
est un éditeur de code _open source_, multilingue et multi plate-forme
dont le développement a été initié par la société Microsoft.
Le logiciel est basé sur Electron, un environnement d'exécution
...
Il existe en plusieurs éditions dont la _Community Edition_, qui est gratuite.
...
Liste de diffusion ([URL]#https://groups.google.com/group/nodejs-sec#)::
  Pour recevoir l'alerte par courriel (moins d'un par{nbsp}mois).
...
applications Node.
Celles-ci risquent notamment d'être vulnérables du fait des modules `npm` dont elles
sont dépendantes – en plus des vulnérabilités de{nbsp}Node.
...
vers une version suivante, maintenue à long terme,{nbsp}elle.
