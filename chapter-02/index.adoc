...
Il se peut qu'une nouvelle branche stable soit sortie depuis l'édition de cet ouvrage, par exemple Node {nodeNextVersion}.
...
<!--
Il manque une petite phrase pour dire que tu parles de NVM un peu plus loin
-->
...
<!--
Peut-être serait-il pertinent de mettre en cache le gist, ou d'utiliser un raccourcisseur d'URL pour y accéder, de manière à pouvoir maintenir les liens pendant quelques mois / années ?
-->
...
.Installation de _nvm_ et de Node {nodeVersion} sur un environnement Linux.
----
curl https://raw.github.com/creationix/nvm/v0.23.0/install.sh | sh
nvm install 0.12
...
<!--
Pour info, j'utilise https://github.com/coreybutler/nvm-windows qui marche bien aussi.
Je jeterai un oeil à ces alternatives :)
-->
...
Une image Docker aura la même recette d'installation même si celle-ci est exécutée sous Linux, OSX ou encore Windows.

Lancer un interpréteur Node _sans polluer_ le système hôte revient à exécuter l'image suivante :

----
docker run -ti --rm node:0.12-slim
----

Nous nous nous repencherons sur Docker dans le link:../chapter-05/index.adoc[chapitre 5], dans la section _Déploiement_.
...
Ce site référence l'état de l'implémentation d'ECMAScript 6 sur différentes plates-formes, dont Node :
...
Ce résumé des primitives et fonctionnalités principales d'_ECMAScript 5_ a pour but de vous apprendre ou rappeler la syntaxe à disposition pour écrire des programmes pour Node.
...
La documentation et les informations relatives à son utilisation sont accessibles sur [URL]#https://npmjs.org/package/comma-separated-values#.
...
----
var fs = require('fs');
var csv = require('comma-separated-values'); // <1>

fs.readFile('./data/books.csv', function(err, binaryContent){
  var csvFile = new csv(String(binaryContent), { header: true, delimiter: ';' }); // <2>
