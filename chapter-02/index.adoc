...
Vous pouvez les déclarer de plusieurs manières
...
----
<1> Retourne `'function'` ;
<2> Retourne `false` — la valeur étant nulle ;
<3> Retourne `[ 1, 3 ]` — un tableau filtré des valeurs nulles ou égales à `undefined` ;
<4> Lève une exception car la constante `values` n'est pas définie dans ce _scope_.
