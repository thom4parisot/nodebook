...
Typiquement, une personne développant en Node écrit du code se basant sur les API à disposition. Ce code est lu par le _runtime_ Node qui le transmet à la VM JavaScript. Enfin, VM traduit le programme en langage machine (_bytecode_) avant que le programme soit effectivement exécuté par le processeur.
...
_io.js_ est un _fork_ communautaire de Node.

Les objectifs du projet _io.js_ sont doubles :

- offrir à la communauté Node une gestion de la plateforme davantage transparente, inclusive et ouverte ;
- fournir une plateforme technique plus moderne, une version de v8 plus récente ainsi qu'une inclusion et un suivi du langage ECMAScript plus rapide.

Idéalement, _io.js_ souhaite reverser les fruits de son travail à la plateforme Node mais en février 2015, les rapports entre Joyent et _io.js_ sont obscurs.
...
L'entreprise a pris en charge l'hébergement de la base de données du registre _npm_ de 2010 jusqu'en décembre 2013.
...
[CAUTION]
====
Mettre à jour cette partie, notamment en regard de <<node-foundation,Node.js Foundation>>.
====
...
L'écosystème de modules hébergé sur _npmjs.com_ est le fruit de ses différents contributeurs.
...
*Et ces modules sont une des forces principales de Node*.
...
[CAUTION]
====
Ceci est moins vrai depuis la v4 — idem pour les numéros de version.
====
...
- les versions `0.8.x`, `0.10.x`, `4.x` représentent les évolutions stables de Node ;
- les versions `0.7.x`, `0.9.x`, `3.1.x` représentent les branches de développement qui deviendront les futures branches stables.
