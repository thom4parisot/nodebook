...
HTTP/2 a été initié par Google sous le nom de protocole SPDY (prononcer _speedy_).
====
...
de mathématiques.
Si son but initial était de devenir professeur dans cette matière, il prend la
...
_Eurêka_{nbsp}!
La machine virtuelle JavaScript{nbsp}V8 de Google a été libérée en _open source_
...
À l'origine, c'est un module Node créé par Isaacs{nbsp}Schlueter,
...
Isaacs{nbsp}Schlueter est embauché par Joyent en septembre{nbsp}2010.
