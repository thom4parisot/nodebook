...
|===
| Code  | Raison                | Explication
| `101` | `Switching Protocol`  | Le serveur indique basculer vers le protocole spécifié dans l'en-tête `Upgrade`.
...
plus confortable.
Nous ouvrons la porte à l'écriture de <<tests,tests>>, à un
...
indexterm:[npm, test]
indexterm:[application web, test (voir test)]
...
Dans cette section, nous allons appliquer ces connaissances de base au code
précédemment écrit.
Nous vérifierons ainsi que nos exemples fonctionnent comme prévu, mais
peut-être que nous découvrirons aussi des cas limites auxquels nous n'avions pas pensé !

Quand j'écris des tests après avoir écrit la majorité du code,
j'ai tendance à *tester en priorité le code critique*, et celui qui est au
plus proche de l'interface utilisateur.
Je commence donc par les fonctions exportées par un module.
...
Dans un premier temps, nous utiliserons le module `assert` de{nbsp}Node pour
écrire nos assertions.
...
L'idée est de pouvoir la recréer et la détruire à chaque invocation de la commande
`npm test`, sans craindre de générer des effets secondaires, sans perdre de données.
Ça serait dommage de tout perdre si on venait un jour à exécuter les tests
...
indexterm:[base de données, test (voir test)]
