...
----
<1> Le module _express-minify_ transforme les fichiers Sass si leur entête de réponse `Content-Type` vaut `text/x-scss` – cette ligne assigne cette entête aux fichiers dont l'extension est `.scss`.
...
[subs="+quotes"]
.Extrait des entêtes de réponse à l'origine de la création du cookie de session.
----
...
.Entêtes de réponse qui ont un impact sur la sécurité de navigation.
|===
| Entête | Description
...
| `X-Content-Type-Options` | Avec la valeur `nosniff`, les navigateurs bloquent les scripts et les styles qui n'ont respectivement pas l'entête `Content-Type` à `application/javascript` et `text/css`.
