...
<2> Nous accédons aux informations d'un fichier au travers d'un clé, identique à celle de son champ `name` dans le formulaire.
...
nous devons rester vigileant·e sur le contenu des fichiers pour
...
dans une entête de réponse `Set-Cookie`.
...
| `Secure` | Le cookie n'est envoyée que si le document est demandé via HTTPS.
...
<2> Début de la déclaration d'un block nommé `content`.
...
`{%{nbsp}...{nbsp}%}`::
  ...
  Ce block contient une expression dont le contenu est affiché, inclus ou répété
...
<2> Le block `content` défini dans le fichier `list.njk` est injecté à cet endroit du gabarit.
...
C'est vraiment très proche si ce n'est identique à ce nous avons déjà appris
...
Un des premiers élément différenciant est le *branchement d'extensions*.
...
----
<1> Une _middleware_ se branche avec la méthode `app.use()`.
...
`{httpServer}/` et `{httpServer}/rondoudou` produisent des résultats différents.
...
Stockage sur disque::
  _MySQL_, _MSSQL_, _Postgres_ et _MongoDB_ stockent leur données sur disque,
