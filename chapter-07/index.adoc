...
La configuration initiale d'Express configure un serveur HTTP – à la manière
...
Il pourra vous être utile si vous peinez à configurer Express avec le
...
----
<1> La méthode `express.static()` configure le module{nbsp}``npm`` _send_ documenté dans la section <<static>> ce même chapitre.
...
comment afficher les informations - XML, HTML,{nbsp}JSON, CSS, vidéo etc – et si
...
sont stockées en{nbsp}JSON et donc c'est un choix logique pour Node".
...
comme le champ{nbsp}JSON de Postgres).
