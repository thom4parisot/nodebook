...
de générer des effets secondaires, y compris si exécute les tests unitaires
...
<2> La valeur `:memory:` est une valeur spéciale de _SQLite_ — la base est créée en mémoire et disparaît à la fin de l'exécution du script.
...
rappel de la structure de la route pour expliquer le param
...
////
Si votre application écrit des informations en base de données, je recommande alors
que chaque exécution des tests soit responsable de la mise en place de cette
base de données.

Elle doit être absolument indépendante de toute autre base, créée de zéro
en début de tests et supprimée lorsqu'ils leur exécution se termine.
////

tester la requête entrante : facile, y'a juste à donner un paramètre
deux cas ici : un paramètre qui donne une 404 et un paramètre qui retourne des résultats

ça se corse un peu avec la réponse car il y a une dépendance à un objet réponse http
on pourrait écrire le code nous-même mais avec autant de chances d'introduire des bugs/biais

c'est là qu'intervienne les bouchons
on peut injecter du code qui répond comme le ferait un autre objet - on définit un comportement attendu

[CAUTION]
On écrit de manière optimiste : ne sont pas testés les cas de fonctionnement dont nous n'avons pas encore
connaissance dont nous ne sommes pas protégées de tout
