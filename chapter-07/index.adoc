...
pièce ou avec l'aide du framework _express_.
...
Dans un second temps, nous organiserons notre code avec le framework _express_.
...
de connexions réseaux initiées par un client – un navigateur, un automate, etc.
...
[NOTE]
.[RemarquePreTitre]#Documentation# En-têtes HTTP
====
...
Chaque en-tête précise un élément de contexte.
Certaines en-têtes influencent plus que d'autres la réponse du serveur, si
...
Il affichera alors les en-têtes des requêtes et de leur réponse :
...
Chaque requête entrante occupe 1Ko de mémoire – davantage si nous
...
<4> Définition d'un en-tête personnalisé – le préfixe `X-` indique qu'il n'est pas lié au standard HTTP.
...
Notre approche est un peu trop naïve en regardant les en-têtes
...
et que les en-têtes de réponses sont plus fournies qu'avant :
...
Parmi les en-têtes les plus importantes, nous trouvons `Content-Type`,
...
<3> Cette en-tête caractérise la manière d'organiser les données de formulaire – personnellement, je n'arrive jamais à retenir cette valeur et je la copie/colle toujours depuis Stack Overflow ou une documentation technique.
...
avec la commande `curl` ou un formulaire HTML ressemble à quelque chose
de cet ordre-là :
...
[TIP]
.[RemarquePreTitre]#Pratique# E-mails et pièces-jointes
====
Les e-mails utilisent aussi l'encodage `multipart/form-data` pour joindre
des fichiers à un message.
Si vous savez encoder ou décoder des fichiers pour le web, vous savez scripter
l'ajout de pièces-jointes pour les e-mails.
====
...
----
<1> L'en-tête de réponse `Set-Header` crée/modifie la valeur d'un cookie chez le client – ici, le cookie `compteur` est créé avec la valeur `1`.
...
.Extraits des en-têtes d'une réponse HTTP une fois le module _helmet_ configuré.
----
...
en-têtes sont essentielles à la sécurité et pourquoi.
...
----
<1> La méthode `express.static()` configure le module npm _send_ documenté dans la section <<static>> ce même chapitre.
...
– ici, le répertoire `/static`.
Je trouve que ça les rend *plus clairement identifiables* et ça évite toute
ambiguïté avec les autres routes de l'application.
Ça a aussi l'avantage de dissocier les fichiers sources (Sass, Less, etc.)
des fichiers compilés.

Dans le <<../chapter-05/index.adoc#,chapitre 5>>, j'explique comment
...
L'utilisation des doubles tirets (`--`) nous permet de réutiliser le script
...
Cette technique pour être utilisée pour exposer un seul fichier au lieu
d'un répertoire entier.

Enfin, le module npm _express-minify_ ([URL]#https://npmjs.com/express-minify#)
...
----
<1> Le module _express-minify_ transforme les fichiers Sass si leur en-tête de réponse `Content-Type` vaut `text/x-scss` – cette ligne assigne cet en-tête aux fichiers dont l'extension est `.scss`.
<2> Ajout du module comme <<middleware,middleware>>.
<3> Le fichier `main.scss` sera converti en CSS.

Si ce module permet de démarrer plus vite, sans avoir à se familiariser
avec les scripts npm ni même la commande `node-sass`, je vois deux inconvénients
majeurs : les *erreurs sont difficiles à déceler* et elles peuvent se produire
au cas où notre machine de développement est significativement différente du
sereur de production (compilateur, installation manquée, etc.).
Ça représente aussi un *gâchis de ressources* dans la mesure où ces fichiers
ne changent plus une fois mis en ligne ; ça ne veut pas le coup de rajouter
...
----
<1> La connexion à la base de données est asynchrone – l'objet `db` qui permet d'exécuter des requêtes est renvoyé par la promesse.
<2> Exécution d'une requête avec un paramètre issu du <<path,routing>> – le champ `:id`.
...
D'abord l'ouverture d'une connexion qui est asynchrone.
...
afin d'en extraire un ou plusieurs résultats.
...
le caractère `?` renforce la sécurité de notre application.
...
Nous verrons dans la section <<tests>> qu'un des enjeux est de rendre le fichier
de démarrage le plus _fin_ possible.
...
Les sessions utilisateurs centralisent cette mémoire du côté du serveur.
...
Tout stockage qui serait permanent relève des _préférences utilisateurs_.

Le middleware _express-session_ ([URL]#https://npmjs.com/express-session#)
...
----
<1> Configuration du _middleware_ avec un secret qui rend moins prévisible le nommage du <<cookies,cookie>>.
<2> Création d'une donnée de session nommée `pokemon`, de valeur aléatoire.
<3> Affiche un message du genre de `Mon Pokémon en session : Pikachu`.

La même valeur s'affiche si vous ouvrez un nouvel onglet dans le même navigateur
et en vous rendant sur `{serverUrl}/my-pokemon`.
...
Le seul inconvénient à notre exemple est que si nous stoppons et que
nous relançons le serveur, la page `{serverUrl}/my-pokemon` affiche
...
Cette fois, si nous stoppons le serveur et que nous le relançons,
le gestionnaire de session affiche le nom de Pokémon associé à notre identifiant
de session.
La persistance a fonctionné !

[[logs]]
=== Tracer les actions (_logs_)

Consigner les actions (_logging_ en anglais) est une pratique courante
en informatique pour créer une mémoire des actions effectuées par une application.
...
C'est un endroit idéal pour historiser les erreurs avec des indications qui
aideraient à reproduire le problème.
D'ailleurs, l'habitude est à tenir un journal d'erreurs séparé du journal des
événements afin de retrouver plus facilement ces premières.
J'ai tendance à préférer l'installation d'une _sonde_, comme expliqué dans
la section
<<../chapter-06/index.adoc#exceptions,s'informer des erreurs applicatives>>
du <<../chapter-06/index.adoc#,chapitre 6>>.

C'est *à nous de choisir la granularité des informations* historisées.
Nous sommes responsables de rendre ces informations anonymes et de ne pas
mettre d'autre information personnelle qu'un identifiant (numérique ou généré)
...
[CAUTION]
.[RemarquePreTitre]#Attention# Rotation de logs
====
...
tous les X Mégaoctets, tous les X jours.
====

Les modules `npm` pour consigner nos actions fonctionnent comme des `console.log()`
finement configurables.
Les modules _winston_ ([URL]#https://npmjs.com/winston#), _morgan_
([URL]#https://npmjs.com/morgan#) et _bunyan_ ([URL]#https://npmjs.com/bunyan#)
...
Les journaux de sorties se connectent à des logiciels comme _rsyslog_
([URL]#https://rsyslog.com#) ou à des services en ligne comme
_Papertrail_ ([URL]#https://papertrailapp.com#), _Logstash_
([URL]#https://elastic.co/products/logstash#) et _AWS CloudWatch_
...
Nous a composé pas à pas une application web jusqu'à l'organiser avec le framework
_express_.
Maintenant, nous allons réorganiser les composants pour améliorer la
maintenance et la résilience aux changementds de nos applications web.

Notre but ?
*Diminuer la taille du script de lancement*, rendre les composants indépendants
et préparer au mieux les données passées à nos templates pour faciliter
l'écriture de tests et favoriser l'automatisation du déploiement.

=== Modulariser le code des routes

Le défi de lisibilité et de maintenance augmente au fur et à mesure où
...
Nous en profitons pour rendre nos intentions explicites, ce à quoi nous oblige
...
----
<1> Nous déplaçons la configuration et connexion à la base de données dans son propre module.
...
----
<1> Nous réutilisons la base de données de la section <<database>> de ce même chapitre.
...
Je trouve ça agréable de regrouper ce genre de lignes de code dans un même fichier.
Les dépendances aux modules Node et aux modules `npm` deviennent vraiment claires.
...
Le fichier de routes a vocation à réexporter les fonctions de _routing_.
...
Peu importe l'organisation de votre, ce qui compte c'est que cette organisation
vous convienne, soit communicable aux personnes qui utilisent ce code et que
nous puissions suivre le fil logique.

Je vois toutefois apparaître une zone d'ombre : je ne suis pas satisfait
de la connexion à la base de données.
C'est la résolution de la promesse dans la route qui me fait dire ça ; ça ne
...
<2> Le code exécuté par le routeur est la fonction qui est retournée ici.
...
paramètres.
Ça le rend nettement plus lisible – le code est désormais uniquement lié
...
----
<1> Nous utilisons la syntaxe raccourcie de création d'objets pour les exporter en une seule fois.
...
Les paramètres sont les seuls points d'entrées.
...
de notre code en unité réutilisables.
Ils accroissent la qualité, la confiance et la prédictibilité d'une éventuelle
mise en ligne.

Cette section se déroule dans la continuité de la section précédente.
Nous allons préparer et mettre en place pas à pas un environnement de test
pour vérifier les intentions de notre code.

[NOTE]
.[RemarquePreTitre]#Anecdote# Tu peux pas test
====
J'entends régulièrement des témoignages de personnes qui n'écrivent pas de tests
car il n'y a pas assez de temps ou de budget dans leurs projets, parce que les
clients ne les paient pas pour ça ou parce que ça ralentit la cadence de développement.

J'ai écrit mes premiers tests unitaires en rejoignant un
*environnement favorble au travail de qualité* et en
*apprenant au contact d'autres praticien·ne·s*.
J'ai beaucoup appris lors de mon premier _Global Day of Code Retreat_
([URL]#https://www.coderetreat.org#), une journée propice pour explorer des
concepts de programmation au travers des tests unitaires.
====

Les tests unitaires sont destinés à couvrir les différents cas de figures
des entrées et des sorties de nos fonctions.
Nous testons leurs réactions aux arguments pour vérifier que nous obtenons
le résultat attendu.
Je teste en priorité le code critique, partagé (en tant que librairie) et
qui est au plus proche de l'interface utilisateur.

De quoi on a besoin pour tester `routes/books.js` ?
...
Nous avons aussi besoin d'écrire des _assertions_ – des attentes – et nous
allons utiliser le module d'assertion de Node dans un premier temps.
...
C'est normal, c'est parce que le test s'est bien passé.
...
Pour se faire, commençons par simplifier le fichier `src/database.js` en
supprimant toute chemin d'accès écrit en dur :
...
Ça nous ouvre la voie à nous connecter à une base de données différente
...
----
<1> Nous pouvons nous passer de ce test vu que nous allons tester l'exécution de la fonction – les tests seraient redondants.
...
<4> La route configurée retourne bien une fonction – comme toute à l'heure, ça ne suffit pas à affirmer que son fonctionnement correspond à nos attentes.
...
Le chemin d'accès était d'ailleurs exprimé de la manière suivante : `/books/:id`.
...
Je déconseille cette option car ça nous oblige à écrire du code, faillible,
et donc à introduire des biais dans nos propres tests.

Les _librairies d'interception_ se destinent à éviter ce cas de figure.
...
J'ai l'habitude d'utiliser le module npm _sinon.js_ ([URL]#https://npmjs.com/sinon#,
...
L'enjeu est de constater que la connexion à la base de donnée se fasse bien,
que la requête retourne bien la ligne attendue et que ses valeurs soient
bien transmises en réponse.

Malheureusement pour nous : la base de données en mémoire n'a aucune donnée.
Le test échoue donc.
...
les valeurs de retours ou de _callback_ en fonction des paramètres d'appel.
Les *_mocks_ interceptent les résultats* – je n'en parle pas car notre
exemple ne s'y prête pas.
====
...
Ces promesses sont séquentielles et cette syntaxe linéarise leur lecture.
...
<4> Le module sinon propose un ensemble de fonction de vérification (les _matchers_) – ici, nous vérifions que `response.send()` est appelée avec un objet, qui contient au moins une clé `title` dont la valeur est `Design Systems`.

Le test passe !
Et en plus de ça, nous n'avons plus de doute que la fonction `response.send()`
...
Dans ce cas, nous avons eu recourt à un _espion_ pour observer la méthode
`response.status()` d'_express_.
Nous avons fait ainsi car nous n'avions pas à neutraliser son fonctionnement
et seulement d'observer la valeur de son argument.
...
Cette pratique génèrera des effets de bord si nous ne prêtons pas attention
au déroulé des tests.
Ils devraient être remis à zéro entre chaque test.
...
des tests.
J'utilise le module npm _mocha_ ([URL]#https://npmjs.com/mocha#,
...
// ...
----
<1> Import des fonctions qui décrivent respectivement la suite de tests, l'exécution de code avant le démarrage de la suite et l'exécution de code après chaque test.
<2> Import de la fonction de test.
...
d'une fonctionnalité – ici, une route d'application web.
...
----
<1> Déclaration de la suite de test – sa valeur reflète généralement le sujet des tests.
<2> Avant le démarrage des tests, nous créons la base de données et configurons la route.
<3> Après chaque test, nous remettons à zéro l'état de _sinon_ – nos tests n'ont plus la possibilité de se parasiter entre eux.
...
<3> Deuxième test – cette fois, l'espion et le stub ne sont pas affectés par le précédent test.

La suite de tests se fiche pas mal de savoir comment nous testons notre code.
Son objectif est de capturer les messages d'erreur des assertions qui échouent
et de nous les communiquer en contexte.

.Exemple d'une suite de tests _mocha_ qui se termine avec succès.
...
Ce biais naturel fait que nous ne testons pas les cas dont les fonctionnements
...
L'écriture des tests nous force à réfléchir à l'intention et à concevoir
le code de manière à ce qu'il soit testable.
Qu'on adopte ou non cette méthode, nous gagnons en qualité en faisant échouer
les tests, juste pour nous assurer que le passage au vert d'un test n'est pas
un _bug_.
Oui ça arrive.

.Exemple d'une suite de tests _mocha_ dont un test échoue.
image::images/tests-ko.png[width="80%"]

Maintenant que nos tests sont écrits, nous sommes en mesure de
<<deployment,déployer notre application>> automatiquement seulement et seulement
si l'exécution des tests est réussie.

[[deployment]]
=== Déployer automatiquement

Cette section s'inscrit dans la continuité de l'ajout d'automatisation
dans notre manière de fonctionner.
Elle n'est pas donc pas spécifique aux frameworks et est tout à fait valide
...
Si nous ne sommes pas confident·e dans le déploiement, c'est probablement qu'il
y a des nœuds à démêler.
L'automatisation du déploiement et la facilité de sa répétabilité nous permet
de gommer ces frictions petit à petit.
...
L'enjeu des livraisons automatisée est double.
Le premier enjeu est de *faciliter le transport du code vers le serveur de production*.
En cas de bug, le temps passé concerne seulement la résolution du problème et non
la livraison elle-même.
Enfin, l'automatisation sert de *documentation du processus de livraison*.

Nous abordons plus en détails les différents mécanismes techniques
dans le <<../chapter-06/index.adoc#,chapitre 6>>, que ça soit "à la main",
...
texte (méthode, en-têtes, des fois un corps de message) pour que le serveur
...
Le module npm _jshttp_ ([URL]#https://github.com/jshttp#) affiche les en-têtes
...
.Temporalité de l'exécution d'une requête HTTP et des en-têtes de réponse.
...
C'est-à-dire *toute variable dont la valeur provient de l'extérieur*.
Je pense à des données de formulaire, des paramètres d'URL, des chemins d'accès,
des fichiers uploadés mais aussi des scripts qu'ils ou elles injectent dans la page,
...
[URL]#https://npmjs.com/safe-regex#::
  ...
  Ça évite certains bugs qui surchargent le CPU ou qui déjouent les motifs des expressions.
[URL]#https://npmjs.com/dompurify#::
[URL]#https://npmjs.com/xss#::
  Nettoie du HTML d'attributs et de balises non-sollicitées.
  Les injections de scripts et les événements malicieux de clics sont supprimées.
...
Outre les données arbitraire, il faut toujours s'assurer des *permissions*.
L'utilisateur a-t-il ou a-t-elle vraiment le droit d'être là ou il/elle est ?
Le volume du fichier uploadé est-il vraiment cohérent ?

L'utilisation du <<../chapter-03/index.adoc#strict,mode strict>> a l'avantage
de nous protéger de l'exploitation d'anciennes bizarreries de JavaScript.
Ce qui doit planter plante au lieu d'être passé sous silence.

Des fois, même les caractères autorisés suffisent à déjouer notre attention.
...
Un moyen solide de vérifier que nous n'avons pas été déraillé·e en dehors
...
|===
| En-tête | Description
| `X-DNS-Prefetch-Control` | Avec la valeur `off`, le navigateur ne va pas proactivement chercher les informations DNS des ressources contenues dans la page. Nos informations de navigation ne sont pas divulguées à notre insu.
...
| `Strict-Transport-Security` | Lorsque l'option est activée, le navigateur accèdera par défaut à ce site en HTTPS au lieu de HTTP.
| `X-Download-Options` | Avec la valeur `noopen`, Internet Explorer téléchargera un fichier au lieu de tenter de l'ouvrir à même le navigateur – ça peut être exploité pour pirater le navigateur par des fichiers malicieux.
...
| `X-XSS-Protection` | Avec la valeur `1; mode=block`, les navigateurs interrompent le chargement d'une page s'ils détecte une exploitation malicieuse de chargement de données ou de fichier.
|===

[TIP]
.[RemarquePreTitre]#Guide# Open Web Application Security Project (_OWASP_)
====
...
au mieux des attaques.

Ils mettent à disposition un guide spécialisé pour Node à l'adresse
...
Nous en parlons un peu plus en détails dans le <<../chapter-06/index.adoc#,chapitre 6>>,
consacré à l'hébergement et aux déploiements.

Ce mécanisme s'adapte à des résultats en réaction à un événément : _webhook_,
notification, erreur, etc.

Le module npm _micro_ ([URL]#https://npmjs.com/micro#) est un serveur minimaliste
...
----
<1> Nous avons accès à la requête et à la réponse mais nous avons aussi la liberté de renvoyer un résultat en le retournant à la _fonction de callback_.
<2> À la manière d'une <<server,application classique>>, nous la démarrons sur le port HTTP de notre choix.

Un service comme _AWS Lambda_ ([URL]#https://aws.amazon.com/lambda#) pousse
le concept encore plus loin.
La fonction réagit à un événement interne (notification SQS, quelque chose lié
à notre infrastructure, déploiement, etc.) ou à un événement externe (via l'API
ou un autre service ouvert vers l'extérieur).
Elle n'est pas forcément exposée sur le web.
...
----
<1> C'est tout ce qu'il faut pour qu'une _lambda_ retourne un résultat pour qu'il soit utilisable pour l'appelant.

La versatilité de se mécanisme permet de créer une API complète en agrégeant
plusieurs lambda derrière une _portail d'API_ comme _AWS API Gateway_
...
Ces données de sorties sont aussi bien issues des fichiers placés sur notre disque
...
et d'utiliser celui qui nous plait le plus.
...
confiance dans notre code grandissante et pour en automatiser les déploiements.

Le protocole HTTP est une pierre angulaire du web tel que nous l'utilisons.
Le pratiquer avec Node est un bon moyen d'apprendre à le comprendre par la pratique.
