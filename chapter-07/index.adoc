...
- la page PHP est interpétée le temps de la requête, puis tout est mis à la poubelle (en terme de mémoire, donc les valeurs des variables)
...
- pourquoi Apache lancerait pas un script Node comme il ferait avec php ?
  ...
  - mais le coût il est en terme de perf (charger tous les modules, résoudre les chemins, connecter à la base de données, i/o disque/réseau), pour chaque requête, et donc en terme de perception de l'autre côté de la ligne : le spinner il tourne côté navigateur
