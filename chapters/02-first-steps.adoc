...
var modulePath = path.join(__dirname, 'currency-format.js'); <2>
var setupFormatter = require(modulePath); <3>
----
<1> Node interprète d'abord cette ligne…
<2> … puis interprète celle-ci dès que le module `path` est chargé…
<3> … avant de tenter de charger le module suivant _via_ un chemin absolu.
