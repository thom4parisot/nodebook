...
Nous voulons un *programme exécutable* capable de lire un *fichier CSV* défini et d'en *afficher le contenu* de manière intelligible.
...
fs.readFile(FILENAME, functio(err, binaryContent){
  if (err){
    throw err;
  }
