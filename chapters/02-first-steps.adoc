...
- la lecture CSV n'est pas assez robuste ;
...
Ne réinventons pas la roue : *utilisons des modules tiers* !

=== Utiliser des modules tiers

Nous avons évoqués la notion de _modules_ dans les pages précédentes.
Il est désormais temps de plonger plus en détails dans le sujet pour mieux comprendre une des pierres angulaires de Node.

Lorsque la fonction globale `require` est invoquée, plusieurs scénarios peuvent se dérouler, en fonction de la syntaxe employée.

==== `npm`

Le mécanisme privilégié pour installer des modules tiers et leurs dépendances est `npm`.
Il s'agit d'un programme Node 
