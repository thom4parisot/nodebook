...
Ce site référence l'état de l'implémentation d'{esNext} sur différentes plateformes, dont Node :
...
Il également possible de choisir le nombre de nombres après la virgules avec la méthode `toFixed`. +
...
Dans ECMAScript, tout est objet. C'est le _prototype_ qui détermine le comportement du dit objet.
...
L'opérateur `typeof` permet d'identifier un objet. Le problème est que cet opérateur englobe davantage de types qu'espéré : dates, tableaux et expressions régulières, entre autre.
...
Il également possible de surcharger la méthode `toString` d'un objet pour contrôler la conversion de l'objet en chaîne de caractère. +
...
La méthode `bind` est extrêmement puissante car elle permet de modifier le contexte d'exécution de la dite fonction.
