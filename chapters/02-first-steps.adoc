...
----
<1> Retourne `ENTRÉE STANDARD`.

Le concept de _Streams_ sera développé petit à petit au fil de ce chapitre et dans le reste du livre.
C'est un concept puissant mais il y a d'autres choses plus simples à apprendre d'abord.

Nous venons de voir que `process` est un objet contenant plusieurs attributes précieux.
L'héritage prototypal est utilisé pour le rendre capable d'émettre des évènements via la méthode `.on()`.
