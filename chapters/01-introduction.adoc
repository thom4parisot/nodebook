...
Si vous pouvez faire la même chose que Node dans une technologie / langage que vous connaissez mieux.

Legacy et fonctionnalités qui ne couvriraient pas des besoins existants (type soft tout prêt, il y en a peu car le focus est davantage à l'échelle du module)

Si vos équipes ne sont pas prêtes : il vaut mieux comprendre et régler les raisons du blocage avant de forcer quelconque migration/transition.

Si vous croyez que sa popularité va résoudre vos problèmes internes. 

Crypto, math précision. (ça s'améliore mais il n'y a pas forcément encore tout ce qu'il faut pour des besoins particulièrement pointus).
